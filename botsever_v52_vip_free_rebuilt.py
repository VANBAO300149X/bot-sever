...
BOT_FREE_ISSUED_FILE = str(BOT_DATA_DIR / "telegram_free_issued.json")
...
BOT_FREE_ISSUED = _load_json_file(BOT_FREE_ISSUED_FILE, {})
...
def _free_key_used(key):
    ...
    return any(str(k).strip().upper() == target for k in BOT_FREE_USED)


def _save_free_issued():
    return _save_json_file(BOT_FREE_ISSUED_FILE, BOT_FREE_ISSUED)


def _register_free_key_issued(record):
    """Lưu Key FREE đã phát hành để /login có thể tự kiểm tra độc lập."""
    if not isinstance(record, dict):
        return False
    key = str(record.get("key", "")).strip().upper()
    chat_id = str(record.get("chat_id", "")).strip()
    if not key or not chat_id:
        return False
    BOT_FREE_ISSUED[key] = {
        "chat_id": chat_id,
        "created_at": float(record.get("created_at", _now_ts()) or _now_ts()),
        "expires_at": float(record.get("expires_at", 0) or 0),
        "device_id": str(record.get("device_id", "")),
        "telegraph_url": str(record.get("telegraph_url", "")),
        "link4m_url": str(record.get("link4m_url", "")),
        "link": str(record.get("link", "")),
    }
    return _save_free_issued()


def _get_free_issued_record(key):
    """Tìm Key FREE đã phát hành, kể cả khi BOT_PENDING không còn."""
    target = str(key or "").strip().upper()
    if not target:
        return None
    for stored_key, rec in BOT_FREE_ISSUED.items():
        if str(stored_key).strip().upper() != target or not isinstance(rec, dict):
            continue
        expires_at = float(rec.get("expires_at", 0) or 0)
        return {**rec, "expired": bool(expires_at and _now_ts() >= expires_at)}
    return None
...
def handle_message(msg):
    ...
    if cmd == "🆓 NHẬP KEY FREE":
        # Chỉ đổi từ VIP sang FREE; giữ nguyên link FREE đang chờ nếu đã có.
    ...
    if cmd.lower().startswith("/login"):
        ...
        # /login không có key: chỉ chuyển sang chế độ chờ key,
        # không được xóa pending FREE đang chứa expected key.
        if len(parts) < 2 or not parts[1].strip():
            pending = _bot_pending_record(chat_id)
            if _free_pending_is_valid(pending):
                send_message(
                    chat_id,
                    "🔑 HÃY GỬI KEY FREE VỪA LẤY TỪ TELEGRAPH.",
                )
            else:
                BOT_PENDING.pop(str(chat_id), None)
                ...
                send_message(
                    chat_id,
                    "🔑 Hãy gửi Key FREE ngay tin nhắn tiếp theo để xác thực.",
                )
            return

        user_key = parts[1].strip()
        _handle_free_key_login(chat_id, user_key)
        return
    ...
    # Người dùng có thể gửi thẳng Key lấy từ Telegraph, không bắt buộc gõ /login.
    pending_free = _bot_pending_record(chat_id)
    known_free = _get_free_issued_record(cmd) if not cmd.startswith("/") else None
    if (_free_pending_is_valid(pending_free) or known_free) and not cmd.startswith("/"):
        _handle_free_key_login(chat_id, cmd)
        return
...
def handle_callback(query):
    ...
    if data == "home":
        ...
        # Callback FREE cũng chỉ hủy trạng thái VIP; không xóa link FREE đang chờ.
        _handle_free_key_request(chat_id)
    elif data == "auth:vip":
...
def _free_pending_is_valid(rec, now=None):
    """Kiểm tra record FREE đã tạo nhưng chưa kích hoạt."""
    if not isinstance(rec, dict):
        return False
    if str(rec.get("mode", "")).lower() != "free":
        return False
    if not str(rec.get("key", "")).strip():
        return False
    if not str(rec.get("link", "")).strip():
        return False
    now = _now_ts() if now is None else float(now)
    expires_at = float(rec.get("expires_at", 0) or 0)
    return expires_at > now and not _free_key_used(rec.get("key"))


def _generate_free_key():
    """Tạo Key FREE mới, tránh trùng các key đã từng kích hoạt."""
    for _ in range(10):
        key = f"FREE-{secrets.token_hex(4).upper()}"
        if not _free_key_used(key):
            return key
    raise RuntimeError("Không tạo được Key FREE duy nhất.")


def _shorten_link4m(telegraph_url):
    """Telegraph -> Link4m."""
    if not LINK4M_TOKEN:
        raise RuntimeError("Thiếu LINK4M_TOKEN.")
    ...
    if response.status_code != 200:
        raise RuntimeError(f"Link4m HTTP {response.status_code}")

    try:
        data = response.json()
    except ValueError as exc:
        raise RuntimeError("Link4m trả về JSON không hợp lệ.") from exc

    if not isinstance(data, dict) or str(data.get("status", "")).lower() != "success":
        raise RuntimeError(
            str(data.get("message", "Link4m API không trả về success."))
            if isinstance(data, dict)
            else "Link4m API không trả về success."
        )

    shortened = str(data.get("shortenedUrl") or data.get("shortened_url") or "").strip()
    if not shortened.startswith(("http://", "https://")):
        raise RuntimeError("Link4m không trả về shortenedUrl hợp lệ.")

    return shortened


def _shorten_vuotnhanh(link4m_url):
    """Link4m -> Vượt Nhanh; URL Vượt Nhanh là URL duy nhất gửi cho người dùng."""
    if not VUOTNHANH_TOKEN:
        raise RuntimeError("Thiếu VUOTNHANH_TOKEN.")
    ...
    if response.status_code != 200:
        raise RuntimeError(f"Vượt Nhanh HTTP {response.status_code}")

    try:
        data = response.json()
    except ValueError as exc:
        raise RuntimeError("Vượt Nhanh trả về JSON không hợp lệ.") from exc

    if not isinstance(data, dict) or str(data.get("status", "")).lower() != "success":
        raise RuntimeError(
            str(data.get("message", "Vượt Nhanh API không trả về success."))
            if isinstance(data, dict)
            else "Vượt Nhanh API không trả về success."
        )

    shortened = str(data.get("shortenedUrl") or data.get("shortened_url") or "").strip()
    if not shortened.startswith(("http://", "https://")):
        raise RuntimeError("Vượt Nhanh không trả về shortenedUrl hợp lệ.")

    return shortened


def _create_free_link_chain(chat_id):
    """Tạo chuỗi hoàn chỉnh: Vượt Nhanh -> Link4m -> Telegraph -> Key FREE."""
    created_at = _now_ts()
    device_id = _bot_device_id(chat_id)
    key = _generate_free_key()

    # 1) Tạo Telegraph trước vì đây là đích cuối cùng và chứa chính Key vừa tạo.
    telegraph_url = create_telegraph_page(key, device_id)

    # 2) Bọc Telegraph bằng Link4m.
    link4m_url = _shorten_link4m(telegraph_url)

    # 3) Bọc Link4m bằng Vượt Nhanh.
    vuotnhanh_url = _shorten_vuotnhanh(link4m_url)
...
def _handle_free_key_request(chat_id):
    """Bấm NHẬP KEY FREE -> tạo/cung cấp đúng một link FREE đang chờ."""
    # Chỉ hủy trạng thái VIP; không xóa một link FREE đang chờ của người dùng.
    current_pending = _bot_pending_record(chat_id)
    if current_pending.get("mode") == "vip":
        BOT_PENDING.pop(str(chat_id), None)
        _save_bot_pending()
        _reset_vip_spam(chat_id)
    ...
    # Nếu đang có FREE đang hoạt động thì không tạo thêm link.
    valid, rec = _bot_auth_valid(chat_id)
    if valid and rec and str(rec.get("key_type", "FREE")).upper() == "FREE":
        remaining = max(0, float(rec.get("expires_at", 0) or 0) - _now_ts())
        send_message(
            chat_id,
    ...
    # Nếu đã có link FREE chưa dùng và còn hạn, dùng lại link đó.
    now = _now_ts()
    pending = _bot_pending_record(chat_id)
    if _free_pending_is_valid(pending, now):
        expires = max(0, float(pending["expires_at"]) - now)
        send_message(
            chat_id,
            "🔗 BẠN ĐÃ CÓ LINK LẤY KEY FREE.\n\n"
            "1️⃣ Vượt Nhanh\n"
            "↓\n"
            "2️⃣ Link4m\n"
            "↓\n"
            "3️⃣ Telegraph → hiển thị KEY FREE\n\n"
            f"🚀 LINK VƯỢT NHANH:\n{pending['link']}\n\n"
            "📌 Sau khi Telegraph hiện Key, gửi:\n"
            "/login KEY_CUA_BAN\n\n"
            f"⏰ Link/Key chờ kích hoạt còn: {_format_duration(expires)}",
        )
        return

    # Dọn pending cũ/hết hạn trước khi tạo record mới.
    if pending:
        BOT_PENDING.pop(str(chat_id), None)
        _save_bot_pending()
    ...
    send_message(
        chat_id,
        "⏳ ĐANG TẠO LINK KEY FREE...\n\n"
        "🔄 Vượt Nhanh → Link4m → Telegraph",
    )

    try:
        record = _create_free_link_chain(chat_id)

        # Chỉ lưu pending sau khi cả 3 bước đều thành công.
        BOT_PENDING[str(chat_id)] = record
        if not _save_bot_pending():
            BOT_PENDING.pop(str(chat_id), None)
            _save_bot_pending()
            raise RuntimeError("Không lưu được trạng thái Key FREE.")

        # Lưu riêng Key đã phát hành để /login KEY có thể tự kiểm tra
        # ngay cả khi pending bị mất hoặc bot vừa khởi động lại.
        if not _register_free_key_issued({**record, "chat_id": chat_id}):
            _admin_only_exception_log(
                chat_id,
                "Không lưu được Key FREE đã phát hành",
                RuntimeError("BOT_FREE_ISSUED không thể lưu."),
            )

        send_message(
            chat_id,
            "✅ ĐÃ TẠO LINK LẤY KEY FREE THÀNH CÔNG!\n\n"
            "🔗 Chuỗi link:\n"
            "🚀 Vượt Nhanh\n"
            "   ↓\n"
            "🔗 Link4m\n"
            ...
            "📄 Telegraph\n"
            "   ↓\n"
            "🔑 Hiển thị đúng KEY FREE bot vừa tạo\n\n"
            f"👉 LINK VƯỢT NHANH:\n{record['link']}\n\n"
            "📌 Khi hoàn tất, Telegraph sẽ hiển thị Key.\n"
            "📌 Gửi Key cho bot bằng:\n"
            "/login KEY_CUA_BAN\n\n"
            "⏰ Key FREE có hiệu lực 24 giờ sau khi kích hoạt.\n"
            "⚠️ Key chỉ kích hoạt 1 lần.",
        )
    except Exception as exc:
        # Không giữ Key/link dở dang nếu bất kỳ mắt xích nào thất bại.
        BOT_PENDING.pop(str(chat_id), None)
        _save_bot_pending()
        _admin_only_exception_log(chat_id, "Lỗi tạo chuỗi Link FREE", exc)
        send_message(
            chat_id,
            "❌ KHÔNG TẠO ĐƯỢC LINK KEY FREE.\n\n"
            "Hệ thống chưa gửi link vì chuỗi "
            "Vượt Nhanh → Link4m → Telegraph chưa hoàn tất.\n"
            "Vui lòng bấm 🆓 NHẬP KEY FREE để thử lại.",
            _auth_menu_markup(),
        )


def _handle_free_key_login(chat_id, user_key):
    """Tự động kiểm tra Key FREE; Key sai luôn báo Key sai."""
    user_key = str(user_key or "").strip().upper()
    if not user_key:
        send_message(chat_id, "❌ KEY FREE KHÔNG ĐƯỢC ĐỂ TRỐNG.", _auth_menu_markup())
        return

    if _free_key_used(user_key):
        send_message(
            chat_id,
            "❌ KEY FREE KHÔNG HỢP LỆ.\n\n"
            "⛔ Key này đã được kích hoạt trước đó và không thể sử dụng lại.",
            _auth_menu_markup(),
        )
        return

    pending = _bot_pending_record(chat_id)
    pending_valid = _free_pending_is_valid(pending)
    issued = _get_free_issued_record(user_key)

    # Có pending của chính user: đối chiếu trực tiếp với Key vừa nhập.
    if pending_valid:
        expected_key = str(pending.get("key", "")).strip().upper()
        if user_key != expected_key:
            send_message(
                chat_id,
                "❌ KEY FREE KHÔNG ĐÚNG.\n\n"
                "🔐 Key bạn nhập không trùng với Key đang được cấp. "
                "Hãy mở lại Telegraph và lấy đúng Key.",
            )
            return
        source_record = pending

    # Pending không còn nhưng Key đã được bot phát hành: vẫn xác thực được.
    elif issued and not issued.get("expired"):
        owner = str(issued.get("chat_id", "")).strip()
        if owner and owner != str(chat_id):
            send_message(
                chat_id,
                "❌ KEY FREE KHÔNG HỢP LỆ.\n\n"
                "⛔ Key này được cấp cho tài khoản Telegram khác.",
                _auth_menu_markup(),
            )
            return
        source_record = issued

    elif issued and issued.get("expired"):
        send_message(
            chat_id,
            "❌ KEY FREE ĐÃ HẾT HẠN.\n\n"
            "👉 Bấm 🆓 NHẬP KEY FREE để tạo Key mới.",
            _auth_menu_markup(),
        )
        return

    else:
        # Quan trọng: Key nhập sai không còn bị báo "chưa tạo link FREE".
        send_message(
            chat_id,
            "❌ KEY FREE KHÔNG ĐÚNG.\n\n"
            "🔐 Key bạn nhập không tồn tại hoặc không phải Key do bot cấp.\n"
            "👉 Kiểm tra lại Key trên Telegraph rồi nhập lại.",
        )
        return

    if str(source_record.get("key", "")).strip().upper() != user_key:
        send_message(chat_id, "❌ KEY FREE KHÔNG ĐÚNG.\n\n🔐 Vui lòng nhập đúng Key đang hiển thị trên Telegraph.")
        return

    link_used = (
        source_record.get("link")
        or source_record.get("link4m_url")
        or source_record.get("telegraph_url")
    )
    if not _mark_free_key_used(user_key, chat_id, link_used):
        _admin_only_exception_log(
            chat_id,
            "Không lưu được trạng thái Key FREE đã dùng",
            RuntimeError("BOT_FREE_USED không thể lưu."),
        )
        send_message(
            chat_id,
            "❌ Không thể lưu trạng thái Key FREE. Vui lòng thử lại sau.",
            _auth_menu_markup(),
        )
        return
    ...
    send_message(
        ...
        "🆓 Quyền: LOGIC FREE\n"
        "⏰ Hiệu lực: 24 giờ\n\n"
        "Bạn có thể bấm 🧰 CHỌN TOOL để sử dụng.",
        home_markup(chat_id),
    )
...
def create_telegraph_page(key, device_id):
    """Tạo trang Telegraph đích cuối cùng và hiển thị đúng Key FREE vừa tạo."""
    ...
    if not key:
        raise ValueError("Key FREE rỗng.")
    if not ACCESS_TOKEN:
        raise RuntimeError("Thiếu ACCESS_TOKEN Telegraph.")

    content = json.dumps([
        {
            "tag": "h3",
            "children": ["🔑 VBTOOL FREE KEY"],
        },
        {
            "tag": "p",
            "children": [f"📱 Device ID: {device_id}"],
        },
        {
            "tag": "p",
            "children": [f"🔐 KEY: {key}"],
        ...
        {
            "tag": "p",
            "children": ["⏰ Hiệu lực: 24 giờ kể từ lúc kích hoạt."],
        },
        {
            "tag": "p",
            "children": ["⚠️ Key chỉ sử dụng cho đúng Telegram ID đã yêu cầu."],
        },
        {
            "tag": "p",
            "children": ["⚠️ Key FREE chỉ được kích hoạt 1 lần."],
        },
    ])

    payload = {
        "access_token": ACCESS_TOKEN,
        "title": "VBTOOL FREE KEY",
        "author_name": "VBTOOL",
        "content": content,
    ...
    if response.status_code != 200:
        raise RuntimeError(f"Telegraph HTTP {response.status_code}")

    try:
        result = response.json()
    except ValueError as exc:
        raise RuntimeError("Telegraph trả về dữ liệu không hợp lệ.") from exc

    if not isinstance(result, dict) or not result.get("ok"):
        raise RuntimeError(
            str(result.get("error", "Telegraph không tạo được trang."))
            if isinstance(result, dict)
            else "Telegraph không tạo được trang."
        )

    result_data = result.get("result") or {}
    telegraph_url = str(result_data.get("url") or "").strip()
    if not telegraph_url.startswith(("http://", "https://")):
        raise RuntimeError("Telegraph không trả về URL hợp lệ.")
