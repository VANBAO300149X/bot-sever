    ...
    if upper_cmd.startswith("/DUYET"):
        ...
        try:
            send_message(
                ...
                f"📦 Gói: {order.get('label', '-') }\n"
                f"🧾 Đơn: {order_id}\n\n"
                "✅ Key đã được kích hoạt cho Telegram của bạn.\n"
                "👉 Nhấn/giữ trực tiếp vào Key để sao chép giống thông tin ngân hàng.\n"
                "Bạn có thể bấm /start để sử dụng.",
                None,
                "HTML",
            )
        except Exception:
...
def _payment_text(order):
    ...
    return (
        ...
        "📋 <b>Nhấn vào NỘI DUNG để sao chép.</b>\n"
        "⚠️ Chuyển đúng số tiền và đúng nội dung trên.\n"
...
def handle_callback(query):
    ...
    if data == "home":
        ...
        if not ok:
            ...
            send_message(
                chat_id,
                "⏳ Yêu cầu duyệt đơn này đã được gửi cho Admin.\n\n"
                + _format_order(BOT_ORDERS.get(order_id, {}), show_key=False),
            )
        else:
            send_message(
                chat_id,
                "📨 ĐÃ GỬI YÊU CẦU DUYỆT CHO ADMIN\n\n"
                + _format_order(result, show_key=False)
                + "\n⏳ Vui lòng chờ Admin kiểm tra giao dịch và duyệt Key.",
                home_markup(chat_id),
            )
