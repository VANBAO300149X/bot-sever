...
def _create_free_key_link_like_vbtool(chat_id):
    """Tạo Key FREE theo đúng chuỗi trong vbtoolvip2k999_final.py:
    random key -> Telegraph -> Link4m -> Vượt Nhanh.
    Mỗi Telegram ID giữ riêng key/link trong BOT_PENDING.
    """
    device_id = _bot_device_id(chat_id)
    ...
    if not telegraph_url:
        raise RuntimeError("Không thể tạo trang Telegraph")

    # Bước 1: Link4m — giống file gốc, không fallback sang URL khác.
    params_4m = {
        "api": LINK4M_TOKEN,
        "url": telegraph_url,
    }
    ...
    link4m_url = str(data1.get("shortenedUrl") or "").strip()
    if not link4m_url:
        raise RuntimeError("Link4m không trả về shortenedUrl")

    # Bước 2: Vượt Nhanh — giống file gốc, bắt buộc API success.
    params_vn = {
        "api": VUOTNHANH_TOKEN,
        "url": link4m_url,
    }
    ...
    if data2.get("status") != "success":
        raise RuntimeError("Vuotnhanh API không trả về success")

    final_url = str(data2.get("shortenedUrl") or "").strip()
    if not final_url:
        raise RuntimeError("Vuotnhanh không trả về shortenedUrl")

    return key, final_url, device_id
...
def _handle_free_key_request(chat_id):
    ...
    # Nếu đã có link FREE đang chờ thì dùng lại link cũ, tránh tạo key/link mới liên tục.
    now = _now_ts()
    pending = _bot_pending_record(chat_id)
    if (
        pending.get("mode") == "free"
        and pending.get("link")
        and pending.get("key")
        and now - float(pending.get("created_at", 0) or 0) < 300
    ):
        send_message(
            chat_id,
    ...
    try:
        key, final_url, device_id = _create_free_key_link_like_vbtool(chat_id)
        ...
        send_message(
            chat_id,
            "❌ Không tạo được link FREE. Vui lòng thử lại sau.",
        )
