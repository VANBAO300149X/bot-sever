...
BOT_SPAM = {}          # chat_id -> {"FREE": [timestamps], "VIP": [timestamps]}
...
def _ban_record(chat_id, key_type=None):
    """Lấy BAN theo phạm vi: GLOBAL, FREE hoặc VIP."""
    root = BOT_BANS.get(str(chat_id))
    if not isinstance(root, dict):
        return None

    # Dữ liệu BAN cũ: 1 record trực tiếp = BAN chung.
    if key_type is None:
        if any(k in root for k in ("GLOBAL", "FREE", "VIP")):
            return root.get("GLOBAL") if isinstance(root.get("GLOBAL"), dict) else None
        return root

    key_type = str(key_type).upper()
    if any(k in root for k in ("GLOBAL", "FREE", "VIP")):
        rec = root.get(key_type)
        if isinstance(rec, dict):
            return rec
        rec = root.get("GLOBAL")
        return rec if isinstance(rec, dict) else None

    # Legacy record: chỉ coi là BAN chung nếu không xác định được loại.
    reason = str(root.get("reason", "")).upper()
    if key_type == "VIP" and "KEY VIP" in reason:
        return root
    if key_type == "FREE" and "KEY FREE" in reason:
        return root
    return None


def _ban_remaining(chat_id, key_type=None):
    rec = _ban_record(chat_id, key_type)
    ...
    if remaining <= 0:
        root_key = str(chat_id)
        root = BOT_BANS.get(root_key)
        if isinstance(root, dict) and any(k in root for k in ("GLOBAL", "FREE", "VIP")) and key_type:
            root.pop(str(key_type).upper(), None)
            if not any(root.get(k) for k in ("GLOBAL", "FREE", "VIP")):
                BOT_BANS.pop(root_key, None)
        else:
            BOT_BANS.pop(root_key, None)
...
def _register_auth_spam(chat_id, key_type, reason="spam"):
    """Đếm spam riêng cho FREE/VIP và chỉ BAN đúng loại key đó."""
    chat_key = str(chat_id)
    key_type = str(key_type or "FREE").upper()
    if key_type not in ("FREE", "VIP"):
        key_type = "FREE"
    now = _now_ts()

    with BOT_AUTH_LOCK:
        bucket = BOT_SPAM.get(chat_key)
        if not isinstance(bucket, dict):
            # Tương thích dữ liệu RAM cũ nếu có.
            bucket = {"FREE": [], "VIP": []}
            BOT_SPAM[chat_key] = bucket
        bucket.setdefault("FREE", [])
        bucket.setdefault("VIP", [])
        arr = [t for t in bucket[key_type] if now - t <= BOT_SPAM_WINDOW]
        arr.append(now)
        bucket[key_type] = arr
        count = len(arr)
        ...
        bans = BOT_BANS.get(chat_key)
        if not isinstance(bans, dict) or not any(k in bans for k in ("GLOBAL", "FREE", "VIP")):
            # Chuyển BAN cũ trực tiếp thành cấu trúc mới, không mất BAN Admin cũ.
            if isinstance(bans, dict) and bans.get("manual"):
                bans = {"GLOBAL": bans}
            else:
                bans = {}
            BOT_BANS[chat_key] = bans
...
def _auth_spam_count(chat_id, key_type):
    bucket = BOT_SPAM.get(str(chat_id), {})
    if isinstance(bucket, dict):
        now = _now_ts()
        key_type = str(key_type).upper()
        return len([t for t in bucket.get(key_type, []) if now - t <= BOT_SPAM_WINDOW])
    return 0


def _reset_auth_spam(chat_id, key_type=None):
    chat_key = str(chat_id)
    if key_type is None:
        BOT_SPAM.pop(chat_key, None)
        return
    bucket = BOT_SPAM.get(chat_key)
    if isinstance(bucket, dict):
        bucket[str(key_type).upper()] = []
        if not bucket.get("FREE") and not bucket.get("VIP"):
            BOT_SPAM.pop(chat_key, None)
...
def _bot_banned_message(chat_id, key_type=None):
    """Thông báo BAN đúng phạm vi FREE/VIP; key_type=None chỉ kiểm tra BAN chung."""
    remaining = _ban_remaining(chat_id, key_type)
    rec = _ban_record(chat_id, key_type)
    if remaining == -1:
        if key_type:
            reason = str((rec or {}).get("reason", "Spam Key " + str(key_type).upper())).strip()
            return (
                f"⛔ KEY {str(key_type).upper()} ĐÃ BỊ BAN VĨNH VIỄN.\n\n"
                f"🔒 Lý do: {reason}"
            )
        return "⛔ TÀI KHOẢN ĐÃ BỊ BAN VĨNH VIỄN."
    if remaining > 0:
        prefix = f"KEY {str(key_type).upper()}" if key_type else "TÀI KHOẢN"
        return (
            f"⛔ {prefix} ĐÃ BỊ BAN DO SPAM.\n\n"
...
def _set_bot_auth(chat_id, key, key_type, expires_at, server_device_id=None, duration_hours=None):
    ...
    with BOT_AUTH_LOCK:
        ...
        _save_json_file(BOT_AUTH_FILE, BOT_AUTH)
    _reset_auth_spam(chat_id, key_type)
    return rec
...
def start_tool(chat_id, key):
    ban = _bot_banned_message(chat_id, None)
    if ban:
...
def handle_message(msg):
    ...
    if cmd == "👑 NHẬP KEY VIP":
        ban = _bot_banned_message(chat_id, "VIP")
        if ban:
            send_message(chat_id, ban, _auth_menu_markup())
            return
    ...
    if cmd in ("/start", "/menu", "/tools"):
        # Mỗi Telegram ID có một hồ sơ key riêng trong telegram_auth.json.
        # Khi key còn hạn theo hồ sơ local: vào thẳng menu tool.
        # Không gọi verify_vip_key_from_server() lại ở /start vì một số API
        # dùng trạng thái kích hoạt một lần, khiến key vừa kích hoạt bị trả về
        # INVALID/EXPIRED khi kiểm tra lại và làm mất phiên đăng nhập local.
        valid, rec = _bot_auth_valid(chat_id)
        if valid and rec:
            send_message(chat_id, _auth_menu_text(chat_id), home_markup())
        else:
    ...
    if cmd.lower() == '/banlist':
        ...
        active = []
        now = _now_ts()
        for tid, root in list(BOT_BANS.items()):
            if not isinstance(root, dict):
                continue

            # BAN cũ hoặc BAN Admin chung.
            if not any(k in root for k in ("GLOBAL", "FREE", "VIP")):
                remaining = -1 if root.get("permanent") is True else float(root.get('until', 0) or 0) - now
                if remaining == -1 or remaining > 0:
                    active.append((tid, "GLOBAL", remaining, root.get('reason', '')))
                continue

            for ban_type in ("GLOBAL", "FREE", "VIP"):
                rec = root.get(ban_type)
                if not isinstance(rec, dict):
                    continue
                remaining = -1 if rec.get("permanent") is True else float(rec.get('until', 0) or 0) - now
                if remaining == -1 or remaining > 0:
                    active.append((tid, ban_type, remaining, rec.get('reason', '')))

        if not active:
            send_message(chat_id, "📋 DANH SÁCH BAN\n\n✅ Hiện không có người dùng nào đang bị BAN.")
            return
        lines = ["📋 DANH SÁCH BAN", ""]
        for tid, ban_type, remaining, reason in active:
            time_text = "VĨNH VIỄN" if remaining == -1 else _format_duration(remaining)
            type_text = "BAN CHUNG" if ban_type == "GLOBAL" else f"BAN {ban_type}"
            lines.append(f"👤 {tid}\n🔒 {type_text}\n⏰ Còn: {time_text}\n📝 {reason}\n")
        send_message(chat_id, "\n".join(lines).strip())
        return
    ...
    if cmd.lower().startswith("/login"):
        ban = _bot_banned_message(chat_id, "FREE")
        if ban:
            send_message(chat_id, ban, _auth_menu_markup())
            return
        parts = cmd.split(maxsplit=1)
        ...
        if user_key != expected_key:
            banned, seconds = _register_auth_spam(chat_id, "FREE", "Nhập key FREE sai quá số lần cho phép")
            if banned:
                send_message(chat_id, "⛔ Bạn đã bị tạm khóa do spam.\n\n" f"⏰ Thời gian khóa: {_format_duration(seconds)}")
            else:
                remain = max(0, BOT_MAX_AUTH_ATTEMPTS - _auth_spam_count(chat_id, "FREE"))
        ...
        # Ghi dấu trước khi cấp quyền để key không thể được dùng lại sau này,
        # kể cả khi quyền FREE của Telegram ID đã hết 24 giờ.
...
def _verify_bot_vip_key(chat_id, user_key):
    ...
    try:
        ...
        if not result or status in {"REVOKED", "EXPIRED", "DEVICE_MISMATCH", "INVALID", "TELEGRAM_MISMATCH", "TELEGRAM_VIP_ALREADY_ACTIVATED"}:
            banned, _ = _register_auth_spam(chat_id, "VIP", "Nhập Key VIP không hợp lệ 3 lần liên tiếp")
            if banned:
                BOT_PENDING.pop(str(chat_id), None)
                ...
                remain = max(0, BOT_MAX_AUTH_ATTEMPTS - _auth_spam_count(chat_id, "VIP"))
...
def handle_callback(query):
    ...
    ban = _bot_banned_message(chat_id, None)
    if ban:
        answer_callback(callback_id, "Bạn đang bị khóa.")
        send_message(chat_id, ban)
        return
...
def _handle_free_key_request(chat_id):
    ...
    banned, seconds = _register_auth_spam(chat_id, "FREE", "Yêu cầu tạo link FREE quá nhiều")
    if banned:
        send_message(chat_id, "⛔ Bạn đã bị tạm khóa do spam.\n\n" f"⏰ Thời gian khóa: {_format_duration(seconds)}")
        return
