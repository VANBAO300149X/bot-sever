...
def handle_message(msg):
    ...
    if cmd.lower().startswith("/login"):
        ...
        # Xác thực Key FREE với chính Key Server để server ghi nhận
        # used/device/expires_at. Bot không còn tự quyết định thời hạn.
        free_result = verify_free_key_from_server(_bot_device_id(chat_id), user_key)
        if not free_result:
            send_message(chat_id, "❌ Không thể xác thực Key FREE với server. Vui lòng thử lại sau.", _auth_menu_markup())
            return

        status = str(free_result.get("status", "INVALID")).upper()
        if status in {"REVOKED", "EXPIRED", "INVALID", "DEVICE_MISMATCH"} or not free_result.get("success", False):
            if status == "REVOKED":
                msg = "❌ KEY FREE ĐÃ BỊ THU HỒI."
            elif status == "EXPIRED":
                msg = "❌ KEY FREE ĐÃ HẾT HẠN."
            elif status == "DEVICE_MISMATCH":
                msg = "❌ KEY FREE ĐÃ ĐƯỢC KÍCH HOẠT TRÊN THIẾT BỊ KHÁC."
            else:
                msg = "❌ KEY FREE KHÔNG HỢP LỆ."
            send_message(chat_id, msg, _auth_menu_markup())
            return

        duration_hours = free_result.get("duration") or free_result.get("duration_hours") or 24
        try:
            duration_hours = max(1, float(duration_hours))
        except (TypeError, ValueError):
            duration_hours = 24.0

        expires = _parse_iso_ts(free_result.get("expires_at"))
        if not expires:
            expires = _now_ts() + duration_hours * 3600

        _mark_free_key_used(user_key, chat_id, pending.get("link"))
        _set_bot_auth(chat_id, user_key, "FREE", expires, _bot_device_id(chat_id), duration_hours)
        BOT_PENDING.pop(str(chat_id), None)
        _save_bot_pending()
        hours_text = int(duration_hours) if duration_hours.is_integer() else round(duration_hours, 2)
        send_message(chat_id,
                     "✅ XÁC THỰC KEY FREE THÀNH CÔNG!\n\n"
                     "🆓 Quyền: LOGIC FREE\n"
                     f"⏰ Hiệu lực: {hours_text} giờ\n\n"
...
def verify_free_key_from_server(device_id, key):
    """Xác thực Key FREE qua endpoint dùng chung của Key Server."""
    key = str(key or "").strip()
    ...
    try:
        resp = requests.post(
            ...
            json={"device_id": device_id, "key": key},
            timeout=15,
        )
        data = resp.json() if resp.content else {}
        if resp.status_code != 200 or not isinstance(data, dict):
            return {"success": False, "status": f"HTTP_{resp.status_code}"}

        key_type = str(data.get("key_type") or data.get("type") or "").strip().upper()
        if key_type and key_type != "FREE":
            return {"success": False, "status": "INVALID", "message": "Key không phải FREE"}

        return data
    except requests.exceptions.Timeout:
        return {"success": False, "status": "TIMEOUT"}
    except Exception as exc:
        _admin_only_exception_log(os.getenv("VBTOOL_TELEGRAM_ID", ""), "Lỗi xác thực Key FREE", exc)
        return None
...
def _issue_free_key_from_server(chat_id):
    """Lấy 1 Key FREE 24h từ Key Server.

    Server là nơi quản lý/gắn trạng thái Key; bot không tự sinh Key nữa.
    Endpoint /api/issue_key sẽ reserve một Key FREE còn trống và trả key đó.
    """
    try:
        payload = {
            "package": "FREE",
        ...
        data = resp.json() if resp.content else {}
        if resp.status_code != 200 or not isinstance(data, dict):
            raise RuntimeError(f"Key Server HTTP {resp.status_code}")
        if not data.get("success"):
            raise RuntimeError(str(data.get("message") or "Server không còn Key FREE"))

        key = str(data.get("key") or "").strip()
        if not key:
            raise RuntimeError("Key Server không trả về key FREE")

        key_type = str(data.get("key_type") or data.get("type") or "FREE").upper()
        if key_type != "FREE":
            raise RuntimeError(f"Server trả về Key {key_type}, không phải FREE")

        duration = data.get("duration") or 24
        try:
        ...
        _admin_only_exception_log(chat_id, "Lỗi lấy Key FREE từ Key Server", exc)
        return None
...
def _handle_free_key_request(chat_id):
    ...
    # LẤY KEY TỪ SERVER. Bot không tự sinh key bằng secrets.token_hex().
    device_id = _bot_device_id(chat_id)
    send_message(chat_id, "⏳ ĐANG LẤY KEY FREE TỪ SERVER VÀ TẠO LINK...")
    try:
        issued = _issue_free_key_from_server(chat_id)
        if not issued:
            send_message(chat_id, "❌ Key Server không cấp được Key FREE. Vui lòng thử lại sau.", _auth_menu_markup())
            return

        key = issued["key"]
        duration_hours = issued.get("duration_hours", 24)
        telegraph_url = create_telegraph_page(key, device_id)
        if not telegraph_url:
            raise RuntimeError("Không tạo được trang Telegraph lấy Key")

        # Luôn giữ một URL dự phòng hợp lệ. Nếu một dịch vụ rút gọn đang lỗi,
        # người dùng vẫn nhận được link bước trước thay vì làm mất Key đã issue.
        final_url = telegraph_url
        ...
        try:
            ...
            if r1.status_code == 200 and str(data1.get("status", "")).lower() == "success":
                link4m_url = data1.get("shortenedUrl") or data1.get("shortened_url") or data1.get("url")
                if link4m_url:
                    final_url = link4m_url
                    try:
                        ...
                        if r2.status_code == 200 and str(data2.get("status", "")).lower() == "success":
                            vn_url = data2.get("shortenedUrl") or data2.get("shortened_url") or data2.get("url")
                            if vn_url:
                                final_url = vn_url
                    except Exception as exc:
                        _admin_only_exception_log(chat_id, "Lỗi Vượt Nhanh khi tạo link FREE", exc)
            else:
                _admin_only_exception_log(chat_id, "Link4m không tạo được link FREE", RuntimeError(str(data1.get("message") or f"HTTP {r1.status_code}")))
        except Exception as exc:
            _admin_only_exception_log(chat_id, "Lỗi Link4m khi tạo link FREE", exc)
        ...
        BOT_PENDING[str(chat_id)] = {
            ...
            "device_id": device_id,
            "duration_hours": duration_hours,
            "expires_at": issued.get("expires_at") or (now + float(duration_hours) * 3600),
        }
        _save_bot_pending()

        hours_text = int(duration_hours) if float(duration_hours).is_integer() else round(float(duration_hours), 2)
        send_message(chat_id,
                     "🔑 ĐÃ TẠO LINK VƯỢT THÀNH CÔNG:\n\n"
                     f"{final_url}\n\n"
                     "📌 Sau khi có Key, gõ:\n"
                     "/login KEY_CUA_BAN\n\n"
                     f"⏰ Key có hiệu lực trong {hours_text} giờ")
    except Exception as exc:
...
def create_telegraph_page(key, device_id):
    ...
    try:
        ...
        if r.status_code == 200 and isinstance(result, dict) and result.get("ok"):
            return str((result.get("result") or {}).get("url") or "").strip() or None
    except Exception as exc:
        _admin_only_exception_log(os.getenv("VBTOOL_TELEGRAM_ID", ""), "Lỗi tạo trang Telegraph Key FREE", exc)
    return None
