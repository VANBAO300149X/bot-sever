...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8344831081:AAHIHP4ADPnMT48hL5d80Usqt9jMQP_xh2c").strip()
POLL_TIMEOUT = 25
MAX_TEXT = 3900
FLUSH_INTERVAL = 0.40
...
# Mỗi Telegram user có thể chạy nhiều tool song song.
# Khóa session theo (chat_id, tool_key) thay vì chỉ chat_id.
SESSIONS = {}
SESSIONS_LOCK = threading.Lock()
# Tool mà tin nhắn nhập liệu hiện tại sẽ được chuyển tới.
_ACTIVE_TOOL_BY_CHAT = {}
...
class ToolSession:
    ...
    def _flusher(self):
        ...
        with SESSIONS_LOCK:
            session_key = (self.chat_id, self.key)
            if SESSIONS.get(session_key) is self:
                SESSIONS.pop(session_key, None)
            # Nếu tool đang được chọn để nhận input vừa kết thúc, tự chuyển
            # sang một tool khác vẫn đang chạy (nếu có).
            if _ACTIVE_TOOL_BY_CHAT.get(self.chat_id) == self.key:
                _ACTIVE_TOOL_BY_CHAT.pop(self.chat_id, None)
                for (active_chat_id, active_key), active_session in SESSIONS.items():
                    if active_chat_id == self.chat_id and active_session.running:
                        _ACTIVE_TOOL_BY_CHAT[self.chat_id] = active_key
                        break
...
def _session_map_key(chat_id, tool_key):
    return (chat_id, str(tool_key).strip().lower())


def get_session(chat_id, key=None):
    """Lấy session của một tool; key=None thì lấy tool đang active để nhận input."""
    with SESSIONS_LOCK:
        if key is not None:
            return SESSIONS.get(_session_map_key(chat_id, key))
        active_key = _ACTIVE_TOOL_BY_CHAT.get(chat_id)
        if active_key:
            session = SESSIONS.get(_session_map_key(chat_id, active_key))
            if session and session.running:
                return session
            _ACTIVE_TOOL_BY_CHAT.pop(chat_id, None)
        # Fallback: nếu active không còn, lấy một tool bất kỳ còn chạy.
        for (active_chat_id, active_key), session in SESSIONS.items():
            if active_chat_id == chat_id and session.running:
                _ACTIVE_TOOL_BY_CHAT[chat_id] = active_key
                return session
        return None


def get_sessions(chat_id, running_only=False):
    """Lấy tất cả session của một Telegram user."""
    with SESSIONS_LOCK:
        result = []
        for (session_chat_id, _tool_key), session in SESSIONS.items():
...
def set_active_tool(chat_id, key):
    key = str(key).strip().lower()
    with SESSIONS_LOCK:
        session = SESSIONS.get(_session_map_key(chat_id, key))
        if session and session.running:
            _ACTIVE_TOOL_BY_CHAT[chat_id] = key
            return True
        return False
...
def start_tool(chat_id, key):
    ...
    # Cùng một tool đã chạy thì chỉ chuyển tool đó thành ACTIVE để nhận input.
    old = get_session(chat_id, key)
    if old and old.running:
        set_active_tool(chat_id, key)
        send_message(
            chat_id,
            f"🟢 {old.name} đang chạy.\n\n✅ Đã chuyển sang {old.name} để nhận lệnh nhập.",
            home_markup(chat_id),
        )
        return

    session = ToolSession(chat_id, key)
    session_key = _session_map_key(chat_id, key)
    with SESSIONS_LOCK:
    ...
    try:
        ...
        with SESSIONS_LOCK:
            if SESSIONS.get(session_key) is session:
                SESSIONS.pop(session_key, None)
            if _ACTIVE_TOOL_BY_CHAT.get(chat_id) == key:
                _ACTIVE_TOOL_BY_CHAT.pop(chat_id, None)
...
def stop_tool(chat_id, key=None):
    sessions = get_sessions(chat_id, running_only=True)
    if not sessions:
        send_message(chat_id, "ℹ️ Không có tool đang chạy.", home_markup())
        return

    if key is None:
        # Không chỉ còn 1 tool: dừng tool đang ACTIVE để không ảnh hưởng tool khác.
        session = get_session(chat_id)
        if not session:
            send_message(chat_id, "🛑 CHỌN TOOL MUỐN DỪNG\n\nHãy chọn tool muốn dừng:", stop_menu_markup())
            return
        key = session.key
    else:
        key = str(key).strip().lower()
        session = get_session(chat_id, key)
        if not session:
            send_message(
                chat_id,
    ...
    # Chỉ xóa đúng session/tool đã chọn, không ảnh hưởng các tool khác.
    with SESSIONS_LOCK:
        session_key = _session_map_key(chat_id, session.key)
        if SESSIONS.get(session_key) is session:
            SESSIONS.pop(session_key, None)
        if _ACTIVE_TOOL_BY_CHAT.get(chat_id) == session.key:
            _ACTIVE_TOOL_BY_CHAT.pop(chat_id, None)
            for (active_chat_id, active_key), active_session in SESSIONS.items():
                if active_chat_id == chat_id and active_session.running:
                    _ACTIVE_TOOL_BY_CHAT[chat_id] = active_key
                    break
    send_message(chat_id, f"✅ ĐÃ DỪNG {session.name}.", home_markup(chat_id))
...
def show_status(chat_id):
    ...
    active_key = _ACTIVE_TOOL_BY_CHAT.get(chat_id)
    lines = ["🟢 CÁC TOOL ĐANG CHẠY", ""]
    buttons = []
    for session in sessions:
        seconds = int(time.time() - session.started)
        mark = "👉 " if session.key == active_key else "• "
        lines.append(f"{mark}{session.name} | ⏱ {seconds}s | PID {session.proc.pid}")
        buttons.append([{"text": f"⏹️ Dừng {session.name}", "callback_data": f"stop:{session.key}"}])

    lines.extend(["", f"🎯 Tool nhận input hiện tại: {TOOLS.get(active_key, (active_key or '—', ''))[0]}"])
...
def handle_message(msg):
    ...
    if _is_bot_admin(chat_id) and admin_mode in ("admin_mo_ban", "admin_revoke_vip") and not cmd.startswith("/"):
        ...
        if not ok:
            ...
            return
        try:
            running_sessions = get_sessions(int(target_id), running_only=True)
            if running_sessions:
                send_message(int(target_id), "⛔ KEY CỦA BẠN ĐÃ BỊ THU HỒI.")
                for running_session in list(running_sessions):
                    stop_tool(int(target_id), running_session.key)
        except Exception:
            pass
    ...
    if cmd.lower().startswith('/ban') and not cmd.lower().startswith(('/banlist', '/unban')):
        ...
        try:
            ...
            pass
        try:
            for running_session in list(get_sessions(int(target_id), running_only=True)):
                stop_tool(int(target_id), running_session.key)
        except Exception:
            pass
    ...
    session = get_session(chat_id)
    if session and session.running:
        # Chỉ chuyển input tới TOOL ACTIVE. Muốn nhập cho tool khác, bấm lại
        # nút chọn tool đó; tool đang chạy không cần dừng.
        if not session.send_input(text):
            send_message(chat_id,
                         "⚠️ Tool đã đóng hoặc không nhận được dữ liệu.",
                         home_markup())
    else:
        running_sessions = get_sessions(chat_id, running_only=True)
        if running_sessions:
            names = ", ".join(s.name for s in running_sessions)
            send_message(
                chat_id,
                f"🟢 Bạn đang có {len(running_sessions)} tool chạy: {names}.\n\n"
                "👉 Hãy chọn tool muốn tiếp tục nhập.",
                tool_menu_markup(),
            )
        else:
            send_message(chat_id,
