...
# Render: gắn Persistent Disk tại /var/data để giữ Key/ban sau restart/deploy.
# Local/Termux: nếu /var/data không tồn tại thì dùng thư mục hiện tại.
_DEFAULT_DATA_DIR = "/var/data" if Path("/var/data").is_dir() else "."
BOT_DATA_DIR = Path(os.getenv("VBTOOL_DATA_DIR", _DEFAULT_DATA_DIR)).expanduser()
try:
...
BOT_AUTH_FILE = _persistent_path("telegram_auth.json")
BOT_VIP_ACTIVATED_FILE = _persistent_path("telegram_vip_activated.json")  # Legacy
BOT_BAN_FILE = _persistent_path("telegram_bans.json")
...
BOT_PENDING = {}       # chat_id -> {mode, created_at, key, link, admin_action}
...
def _bot_auth_record(chat_id):
    key = str(chat_id)
    with BOT_AUTH_LOCK:
        rec = BOT_AUTH.get(key)
        # Nạp lại từ disk nếu runtime chưa có hồ sơ. /start không xóa Key.
        if not isinstance(rec, dict):
            fresh = _load_json_file(BOT_AUTH_FILE, {})
            if isinstance(fresh, dict) and isinstance(fresh.get(key), dict):
                BOT_AUTH.clear()
                BOT_AUTH.update(fresh)
                rec = BOT_AUTH.get(key)
        if not isinstance(rec, dict):
            return None
...
def tg(method, payload=None, timeout=35):
    ...
    try:
        ...
        raise RuntimeError(f"Telegram network error: {type(exc).__name__}: {exc}") from exc
    try:
        data = r.json()
    except ValueError:
        data = None
    if not r.ok:
        if isinstance(data, dict):
            ...
            raise RuntimeError(f"Telegram HTTP {r.status_code}: {desc}")
...
def handle_message(msg):
    ...
    if cmd.lower().startswith("/login"):
        ...
        pending = BOT_PENDING.get(str(chat_id), {})
...
def _verify_bot_vip_key(chat_id, user_key):
    # Luồng đúng: giới hạn nằm ở từng KEY VIP trên server, không giới hạn
    # số lần một Telegram ID được dùng các KEY VIP khác nhau. Server sẽ
    # trả TELEGRAM_MISMATCH nếu key đã thuộc Telegram ID khác.
    ...
    try:
        ...
        if status == "TELEGRAM_VIP_ALREADY_ACTIVATED":
            BOT_PENDING.pop(str(chat_id), None)
            send_message(
                chat_id,
                "⛔ TELEGRAM ID NÀY ĐÃ TỪNG KÍCH HOẠT KEY VIP.\n\n"
                "❌ Mỗi ID Telegram chỉ được kích hoạt VIP 1 lần.",
                _auth_menu_markup(),
            )
            return

        if not result or status in {"REVOKED", "EXPIRED", "DEVICE_MISMATCH", "INVALID", "TELEGRAM_MISMATCH"}:
...
def bot_main():
    if not BOT_TOKEN:
        raise SystemExit('Chưa đặt BOT_TOKEN trong Render Environment Variables / biến môi trường.')
    print(f"DATA DIR: {BOT_DATA_DIR}", flush=True)
    try:
    ...
    try:
        while not STOP_ALL.is_set():
            try:
                ...
                err_text = str(exc)
                if ("409" in err_text and "Conflict" in err_text) or "terminated by other getUpdates request" in err_text:
                    print(
                        "[POLL] Telegram 409 Conflict: có tiến trình khác đang dùng cùng BOT_TOKEN. "
                        "Chỉ chạy 1 poller (Render hoặc Termux/PC).",
                        file=sys.stderr, flush=True
                    )
                    time.sleep(10)
                    continue

                is_network_error = any(token in err_text for token in (
                    "Connection aborted", "Connection reset", "Connection refused",
                    "RemoteDisconnected", "Read timed out", "ConnectTimeout",
                    "ConnectionError", "ChunkedEncodingError", "Broken pipe",
                    "Max retries exceeded", "ProxyError",
                ))
                if is_network_error:
                    network_error_count += 1
                    now = time.time()
                    if network_error_count == 1 or now - last_network_log >= 15:
                        print(
                            f"[POLL] Mất kết nối Telegram (lần {network_error_count}), đang tự kết nối lại...",
                            file=sys.stderr, flush=True
                        )
                        last_network_log = now
                    time.sleep(min(20, 2 + network_error_count))
                else:
                    print("[POLL ERROR]", exc, file=sys.stderr, flush=True)
                    time.sleep(3)
    finally:
        STOP_ALL.set()
        with SESSIONS_LOCK:
...
DEVICE_ID_FILE = _persistent_path("device_id.txt")
SALT = "VBTOOLKEY"
TEMP_KEY_FILE = _persistent_path(".temp_key.txt")
ACTIVATION_FILE = _persistent_path("activation.dat")
LINK_HISTORY_FILE = _persistent_path("link_history.json")
...
def start_game_flow():
    ...
    # Gửi cấu hình trong một tin nhắn, sau đó gửi trạng thái khởi động VTH
    # thành một tin nhắn riêng trước khi bắt đầu các thread xử lý.
    ...
    with Live(hilo_make_vbtool_layout(state), console=console, refresh_per_second=8, screen=True) as live:
        while True:
            ...
            try:
                ...
                actual_card = hilo__get_game_card(res_create)
                if actual_card is None:
                    state["log_text"] = "SERVER chưa trả lá bài gốc. Đợi ván tiếp theo..."
                    live.update(hilo_make_vbtool_layout(state))
                    time.sleep(2)
                    continue
