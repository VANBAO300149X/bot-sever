...
def is_vip_activated() -> bool:
    """Kiểm tra Key VIP hiện tại từ hồ sơ worker Telegram hoặc activation cục bộ."""
    try:
        ...
        if expires_at:
            try:
                ...
                try:
                    ...
                    if expiry_time.timestamp() <= time.time():
                        return False
                except Exception:
                    pass

        expiry_text = activation.get("expiry_time")
        if expiry_text:
            try:
                expiry_time = datetime.fromisoformat(str(expiry_text).replace("Z", "+00:00"))
                if expiry_time.timestamp() <= time.time():
                    return False
            except Exception:
                pass
