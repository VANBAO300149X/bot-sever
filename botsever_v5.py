...
def send_message(chat_id, text, markup=None):
    ...
    try:
        return tg("sendMessage", payload, timeout=60)
    except Exception as exc:
...
def _handle_free_key_request(chat_id):
    ...
    try:
        ...
        for attempt in range(1, 4):
            try:
                # Link4m đôi lúc nhận kết nối nhưng trả response rất chậm.
                # Timeout chỉ kết thúc lần thử này, tuyệt đối không làm chết worker/bot.
                r1 = requests.get(LINK4M_API, params=params_4m, timeout=(8, 60))
                break
            except (requests.exceptions.Timeout, requests.exceptions.ConnectionError) as exc:
                if attempt < 3:
                    time.sleep(1.5 * attempt)
                    continue
                raise RuntimeError("Link4m không phản hồi sau 3 lần thử") from exc

        if r1 is None:
            raise RuntimeError("Không nhận được phản hồi từ Link4m")
        if r1.status_code != 200:
            raise RuntimeError(f"Link4m HTTP {r1.status_code}")
        try:
            data1 = r1.json()
        except ValueError as exc:
            raise RuntimeError("Link4m trả về dữ liệu không hợp lệ") from exc
