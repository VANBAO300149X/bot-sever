...
BOT_PENDING = _load_json_file(BOT_PENDING_FILE, {})       # chat_id -> {mode, created_at, key, link, admin_action}
...
def _bot_auth_record(chat_id):
    with BOT_AUTH_LOCK:
        ...
        if not isinstance(rec, dict):
            return None
        if rec.get("is_forever"):
            return dict(rec)
        raw_expiry = rec.get("expires_at")
        try:
...
def _set_bot_auth(chat_id, key, key_type, expires_at, server_device_id=None, duration_hours=None, is_forever=False, activated_at=None):
    rec = {
        "key": str(key).strip(),
        "key_type": str(key_type).upper(),
        "expires_at": None if is_forever or expires_at in (None, "") else float(expires_at),
        "is_forever": bool(is_forever),
        "device_id": server_device_id or _bot_device_id(chat_id),
        "duration_hours": duration_hours,
        "activated_at": float(activated_at) if activated_at is not None else _now_ts(),
    }
    with BOT_AUTH_LOCK:
...
def _telegram_server_auth_status(chat_id):
    """Lấy quyền VIP/FREE bền vững từ app.py theo Telegram ID."""
    try:
        resp = requests.get(
        ...
        if data.get("success") and isinstance(data.get("preferred"), dict):
            return True, data["preferred"], "ACTIVE"
        if str(data.get("status", "")).upper() == "NO_ACTIVE_KEY":
            return True, None, "NO_ACTIVE_KEY"
        return True, None, str(data.get("status", "INVALID")).upper()
    except Exception as exc:
        log_debug(f"telegram auth status error: {exc}")
        return False, None, "NETWORK_ERROR"


def _sync_bot_auth_from_server(chat_id):
    """Đồng bộ hồ sơ key local từ app.py, chỉ xóa local khi server xác nhận đã hết quyền."""
    reachable, data, status = _telegram_server_auth_status(chat_id)
    if not reachable:
        return False, _bot_auth_record(chat_id)

    if status == "NO_ACTIVE_KEY" or not data:
        with BOT_AUTH_LOCK:
            BOT_AUTH.pop(str(chat_id), None)
            _save_json_file(BOT_AUTH_FILE, BOT_AUTH)
        return True, None
    ...
    key_type = str(data.get("key_type") or "FREE").upper()
    is_forever = bool(data.get("is_forever", False))
    expires_at = None if is_forever else _parse_iso_ts(data.get("expires_at"))
    if not is_forever and not expires_at:
        return True, None
    activated_ts = _parse_iso_ts(data.get("activated_at"))
    rec = _set_bot_auth(
        chat_id,
        key,
        key_type,
...
def _verify_free_key_from_server(chat_id, key, quiet=False):
    try:
        resp = requests.post(
        ...
        log_debug(f"FREE verify error: {exc}")
        if not quiet:
            return None
        return None


def _register_free_key_on_server(chat_id, key, device_id):
    secret = str(KEY_SERVER_ADMIN_SECRET or "").strip()
    if not secret:
        return False, {"status": "UNAUTHORIZED", "message": "KEY_SERVER_ADMIN_SECRET chưa được cấu hình trên bot"}
    try:
        resp = requests.post(
        ...
        data = resp.json() if resp.content else {}
        return bool(resp.status_code == 200 and isinstance(data, dict) and data.get("success")), data
    except Exception as exc:
        return False, {"status": "NETWORK_ERROR", "message": str(exc)}


def _revoke_key_on_server(telegram_id, key_type=None):
    secret = str(KEY_SERVER_ADMIN_SECRET or "").strip()
    if not secret:
        return False, "KEY_SERVER_ADMIN_SECRET chưa được cấu hình"
    payload = {"telegram_id": str(telegram_id)}
    if key_type:
        payload["key_type"] = str(key_type).upper()
    try:
        resp = requests.post(
...
def _auth_menu_text(chat_id):
    ...
    if valid and rec:
        if rec.get("is_forever") or rec.get("expires_at") in (None, ""):
            remaining_text = "Vĩnh viễn"
        else:
            remaining_text = _format_duration(max(0, float(rec["expires_at"]) - _now_ts()))
        return (
            "✅ ĐÃ XÁC THỰC KEY\n\n"
            f"🔑 Loại Key: {rec.get('key_type', 'FREE')}\n"
            f"⏰ Còn: {remaining_text}\n\n"
...
def start_tool(chat_id, key):
    ...
    if not is_admin_bypass:
        valid, rec = _bot_auth_valid(chat_id)
        if not valid:
            _sync_bot_auth_from_server(chat_id)
            valid, rec = _bot_auth_valid(chat_id)
        if not valid:
...
def handle_message(msg):
    ...
    if _is_bot_admin(chat_id) and admin_mode in ("admin_mo_ban", "admin_revoke_vip", "admin_revoke_free") and not cmd.startswith("/"):
        ...
        server_ok = True
        server_msg = ""
        server_ok, server_msg = _revoke_key_on_server(target_id, key_type)
        if not server_ok:
            send_message(chat_id, f"❌ Không thể thu hồi {key_type}.\n{server_msg}", home_markup(chat_id))
            return
    ...
    if cmd in ("/start", "/menu", "/tools"):
        # Đồng bộ theo Telegram ID từ app.py. Server là nguồn dữ liệu bền vững;
        # chỉ key hết hạn/thu hồi mới bị loại khỏi hồ sơ active.
        _sync_bot_auth_from_server(chat_id)
        valid, rec = _bot_auth_valid(chat_id)
        if valid and rec:
            send_message(chat_id, _auth_menu_text(chat_id), home_markup())
    ...
    if cmd.lower().startswith(("/revoke", "/revoke_key")):
        ...
        server_ok = True
        server_msg = ""
        if requested_type in (None, "VIP", "FREE"):
            server_ok, server_msg = _revoke_key_on_server(target_id, requested_type)
            # Nếu yêu cầu loại cụ thể và server từ chối thì không báo thành công.
            if requested_type in ("VIP", "FREE") and not server_ok:
                send_message(chat_id, f"❌ Không thể thu hồi {requested_type}.\n{server_msg}")
                return
    ...
    if cmd.lower().startswith("/login"):
        ...
        server_result = _verify_free_key_from_server(chat_id, user_key)
        server_status = str((server_result or {}).get("status", "INVALID")).upper()

        if server_result and server_result.get("success"):
            expires = _parse_iso_ts(server_result.get("expires_at"))
            if not expires:
                expires = _now_ts() + float(server_result.get("duration", 24) or 24) * 3600
            _mark_free_key_used(user_key, chat_id)
            _set_bot_auth(
                chat_id,
                user_key,
                "FREE",
                ...
                server_result.get("duration", 24),
                is_forever=bool(server_result.get("is_forever", False)),
                activated_at=_parse_iso_ts(server_result.get("activated_at")),
            )
            BOT_PENDING.pop(str(chat_id), None)
            _save_bot_pending()
            remaining = max(0, expires - _now_ts())
            send_message(chat_id,
                         "✅ XÁC THỰC KEY FREE THÀNH CÔNG!\n\n"
                         "🆓 Quyền: LOGIC FREE\n"
                         f"⏰ Hiệu lực: {_format_duration(remaining)}\n\n"
        ...
        if server_status == "EXPIRED":
            send_message(chat_id, "❌ KEY FREE ĐÃ HẾT HẠN.", _auth_menu_markup())
            return
        if server_status == "TELEGRAM_MISMATCH":
            send_message(chat_id, "❌ KEY FREE ĐÃ ĐƯỢC LIÊN KẾT VỚI TELEGRAM ID KHÁC.", _auth_menu_markup())
            return
        if server_status == "REVOKED":
            send_message(chat_id, "❌ KEY FREE ĐÃ BỊ THU HỒI.", _auth_menu_markup())
            return
        if server_result is None and expected_key == user_key:
            send_message(chat_id, "⚠️ Không thể kết nối máy chủ key. Vui lòng thử lại.", _auth_menu_markup())
            return
...
def _handle_free_key_request(chat_id):
    ...
    # Luôn ưu tiên dữ liệu trung tâm theo Telegram ID để restart bot không làm mất quyền.
    _sync_bot_auth_from_server(chat_id)
    valid, rec = _bot_auth_valid(chat_id)
    ...
    try:
        ...
        registered, register_data = _register_free_key_on_server(chat_id, key, device_id)
        if not registered:
            raise RuntimeError(str(register_data.get("message", "Không lưu được Key FREE trên server")))
...
def bot_main():
    if not BOT_TOKEN:
        raise SystemExit(
            'Chưa đặt BOT_TOKEN.\n'
            'Ví dụ: export BOT_TOKEN="123456:ABC..."'
        )
    try:
...
def _load_bot_activation_from_env() -> Optional[Dict]:
    try:
        ...
        raw_expires_at = rec.get("expires_at")
        expires_at = None
        if not is_forever and raw_expires_at not in (None, ""):
            expires_at = float(raw_expires_at)
            if expires_at and time.time() >= expires_at:
                return None
        expiry_time = (
            datetime.now() + timedelta(days=365000)
            if is_forever
            else (datetime.fromtimestamp(expires_at) if expires_at else (datetime.now() + timedelta(hours=24)))
        )
        return {
            "device_id": get_device_id(),
            "key": rec.get("key", ""),
...
def check_activation_valid(exit_on_expired: bool = True) -> Tuple[bool, Optional[str], bool]:
    # Telegram: không bắt buộc Key để mở tool. Khi user chưa có Key,
    # mặc định dùng quyền FREE; Key vẫn được kiểm tra khi đã có và
    # LOGIC VIP vẫn yêu cầu Key VIP hợp lệ.
    if os.getenv("VBTOOL_TELEGRAM_MODE") == "1":
...
if __name__ == "__main__":
    if len(sys.argv) >= 3 and sys.argv[1] == "--worker":
        # Worker Telegram phải đi qua run_worker() để bật đúng chế độ nhận input
        # từ stdin pipe do ToolSession truyền vào. Trước đây gọi fn() trực tiếp
        # làm VBTOOL_TELEGRAM_MODE chưa được bật, khiến input() chờ bàn phím.
