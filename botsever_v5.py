...
# =========================================================
# app_keys.py hỗ trợ /api/issue_key với:
...
def _purchase_markup():
    return {
        "inline_keyboard": [
            ...
            [{"text": "2 THÁNG • 45.000đ", "callback_data": "buy:2m"},
             {"text": "4 THÁNG • 65.000đ", "callback_data": "buy:4m"}],
...
def _payment_text(order):
    return (
        ...
        f"💰 Số tiền: {_money_vnd(order.get('price', 0))}\n\n"
        f"💳 VÍ: {PAYMENT_WALLET}\n"
        f"👤 CHỦ TÀI KHOẢN: {PAYMENT_ACCOUNT_NAME}\n"
        f"🔢 SỐ TÀI KHOẢN: {PAYMENT_ACCOUNT_NUMBER}\n"
        f"📝 NỘI DUNG: {order.get('payment_content')}\n\n"
...
def _send_payment_instructions(chat_id, order):
    ...
    if qr_path:
        result = send_photo(chat_id, qr_path, caption, _payment_markup(order["order_id"]))
        if result is not None:
            return result
    send_message(chat_id, caption, _payment_markup(order["order_id"]))
    return None
