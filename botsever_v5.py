...
# Telegram worker input: gửi prompt ngay lập tức thay vì giữ prompt trong input()
# vì phía bot đọc stdout theo từng dòng.
_ORIGINAL_INPUT = input

# Loại toàn bộ Rich markup còn sót lại trước khi gửi prompt sang Telegram.
...
# ================== TELEGRAM KEY / ACCESS CONTROL ==================
# Render Web Service/Worker dùng filesystem tạm thời: restart/redeploy có thể
# làm mất các file JSON nếu lưu ở thư mục project. Khi có Persistent Disk
# tại /var/data, ưu tiên lưu toàn bộ trạng thái key vào đó.
...
BOT_EXPIRED_AUTH_FILE = str(BOT_DATA_DIR / "telegram_expired_auth.json")
...
BOT_EXPIRED_AUTH = _load_json_file(BOT_EXPIRED_AUTH_FILE, {})
BOT_PENDING = _load_json_file(BOT_PENDING_FILE, {})


def _save_bot_pending():
    return _save_json_file(BOT_PENDING_FILE, BOT_PENDING)
# Mỗi KEY FREE chỉ được kích hoạt đúng 1 lần, không tái sử dụng sau khi hết hạn.
...
def _admin_only_exception_log(chat_id, context, exc):
    """Ghi lỗi kỹ thuật chi tiết chỉ cho ADMIN; không lộ traceback/exception cho user."""
    try:
        cid = str(chat_id or os.getenv("VBTOOL_TELEGRAM_ID", "")).strip()
        if cid and _is_bot_admin(cid):
...
def _bot_auth_record(chat_id):
    ...
    with BOT_AUTH_LOCK:
        ...
        if expiry and _now_ts() >= expiry:
            # Chỉ báo hết hạn khi thực sự hết hạn; key còn hạn được dùng bình thường.
            BOT_EXPIRED_AUTH[target] = {
            ...
            _save_json_file(BOT_EXPIRED_AUTH_FILE, BOT_EXPIRED_AUTH)
            return None
        return dict(rec)

def _bot_vip_already_activated(chat_id):
    return str(chat_id).strip() in BOT_VIP_ACTIVATED

def _mark_bot_vip_activated(chat_id, key):
    target = str(chat_id).strip()
    with BOT_AUTH_LOCK:
        BOT_VIP_ACTIVATED[target] = {"activated_at": _now_ts(), "key": str(key).strip()}
        _save_json_file(BOT_VIP_ACTIVATED_FILE, BOT_VIP_ACTIVATED)
...
def _set_bot_auth(chat_id, key, key_type, expires_at, server_device_id=None, duration_hours=None):
    target = str(chat_id)
    # expires_at = 0 được dùng cho Key vĩnh viễn.
    try:
        expiry_value = float(expires_at or 0)
    except (TypeError, ValueError):
        expiry_value = 0.0
    rec = {
        "key": str(key).strip(),
    ...
    with BOT_AUTH_LOCK:
        BOT_AUTH[target] = rec
        # Có key mới hợp lệ thì xóa trạng thái hết hạn cũ.
        BOT_EXPIRED_AUTH.pop(target, None)
        _save_json_file(BOT_AUTH_FILE, BOT_AUTH)
        _save_json_file(BOT_EXPIRED_AUTH_FILE, BOT_EXPIRED_AUTH)
...
def _restore_bot_auth_from_server(chat_id):
    """Khôi phục Key FREE/VIP đang còn hạn theo Telegram ID.

    Trả về (server_replied, record).
    - server_replied=True, record có giá trị: tìm thấy Key đang ACTIVE.
    - server_replied=True, record=None: server trả lời nhưng không có Key ACTIVE.
    - server_replied=False: lỗi mạng/cấu hình, cho phép fallback về auth local.
    """
    try:
        if not KEY_SERVER_URL:
            return False, None
        response = requests.post(
            f"{KEY_SERVER_URL}/api/telegram/auth_status",
            json={"telegram_id": str(chat_id)},
            timeout=12,
        )
        ...
        if str(data.get("status", "")).upper() != "ACTIVE" or not data.get("success"):
            return True, None

        rec = data.get("preferred") or data.get("vip") or data.get("free")
        if not isinstance(rec, dict):
            return True, None

        key = str(rec.get("key") or "").strip()
        key_type = str(rec.get("key_type") or "").strip().upper()
        if not key or key_type not in {"FREE", "VIP"}:
            return True, None

        is_forever = bool(rec.get("is_forever", False))
        if is_forever:
            expires_at = 0.0
        else:
            expires_at = _parse_iso_ts(rec.get("expires_at"))
            if not expires_at or expires_at <= _now_ts():
                return True, None
        ...
        saved = _set_bot_auth(
            ...
            key_type,
            expires_at,
            rec.get("device") or _bot_device_id(chat_id),
            rec.get("duration"),
        )
        return True, saved
    except Exception as exc:
        _admin_only_exception_log(chat_id, "Lỗi khôi phục Key từ server", exc)
        return False, None


def _register_free_key_on_server(chat_id, key, device_id=None):
    """Lưu Key FREE vào Key Server để restart bot vẫn nhận lại theo Telegram ID."""
    try:
        if not KEY_SERVER_ADMIN_SECRET or not KEY_SERVER_URL:
            return False, None
        response = requests.post(
            f"{KEY_SERVER_URL}/api/telegram/register_free_key",
            json={
                "key": str(key).strip(),
                "telegram_id": str(chat_id),
                "device_id": str(device_id or _bot_device_id(chat_id)),
                "admin_secret": KEY_SERVER_ADMIN_SECRET,
            },
            timeout=12,
        )
        data = response.json() if response.content else {}
        return response.status_code == 200 and bool(data.get("success")), data if isinstance(data, dict) else None
    except Exception as exc:
        _admin_only_exception_log(chat_id, "Lỗi đăng ký Key FREE lên server", exc)
        return False, None


def _verify_free_key_on_server(chat_id, key):
    """Kích hoạt/xác thực Key FREE trên Key Server."""
    try:
        response = requests.post(
            f"{KEY_SERVER_URL}/api/telegram/verify_free_key",
            json={"key": str(key).strip(), "telegram_id": str(chat_id)},
            timeout=12,
        )
        data = response.json() if response.content else {}
        return response.status_code == 200 and bool(data.get("success")), data if isinstance(data, dict) else None
    except Exception as exc:
        _admin_only_exception_log(chat_id, "Lỗi xác thực Key FREE trên server", exc)
        return False, None
...
def _auth_menu_text(chat_id):
    ...
    expired = BOT_EXPIRED_AUTH.get(str(chat_id))
    if isinstance(expired, dict):
        key_type = str(expired.get("key_type", "FREE")).upper()
        return (
            f"❌ KEY {key_type} ĐÃ HẾT HẠN.\n\n"
...
class ToolSession:
    ...
    def start(self):
        ...
        self.flush_thread = threading.Thread(
            target=self._flusher, daemon=True)
    ...
    def _flusher(self):
        ...
        def _send_part(part):
            # CDTD: tách riêng thông báo lưu cấu hình và bước chọn Logic,
            # kể cả khi stdout được gom vào cùng một lần flush.
            ...
            payload_text = part if self.key in ("vth", "cdtd") else f"{self.name}\n\n{part}"
            markup = None
            if self.key == "cdtd":
                if "9.CHỌN LOGIC" in part:
                    ...
                    markup = {
                        "keyboard": [[{"text": "🆓 LOGIC FREE"}, {"text": "👑 LOGIC VIP"}]],
                        "resize_keyboard": True,
                    ...
                    markup = {
                        ...
                        "resize_keyboard": True,
                        "one_time_keyboard": True,
                        "input_field_placeholder": "Chọn số người...",
                    }
                    ...
                    self.awaiting_cdtd_logic = False
                    markup = {"remove_keyboard": True}
            send_message(self.chat_id, payload_text, markup)
        ...
        while not self.stop_event.is_set() or not self.q.empty():
            ...
            if buf and (
                ...
                for part in chunks(text):
                    # Tách các thông báo khởi động VTH thành 2 tin nhắn Telegram riêng.
                    ...
                    # Tách các bước CDTD thành các tin nhắn Telegram riêng.
        ...
        # Không gửi thông báo kết thúc kèm Exit code ra Telegram.
        if not STOP_ALL.is_set():
            send_message(self.chat_id, f"✅ {self.name} đã kết thúc.", home_markup())
    ...
    def stop(self):
        # Dừng worker êm khi người dùng bấm Ctrl+C / DỪNG TOOL.
        # Chỉ gửi tín hiệu một lần, sau đó chờ worker kết thúc để tránh
        # output/cleanup bị chạy lặp và hạn chế việc Termux hiển thị lại prompt.
        proc = self.proc
        if not proc:
            self.stop_event.set()
            return
...
def start_tool(chat_id, key):
    ...
    if not is_admin_bypass:
        ...
        if key == "xworld" and str(rec.get("key_type", "FREE")).upper() != "VIP":
            send_message(chat_id, "🔒 CANH CODE XWORLD chỉ dành cho KEY VIP.\n\nGõ /KEY_VIP để nhập key VIP.", _auth_menu_markup())
            return
...
def handle_message(msg):
    ...
    # Admin khong bi chan boi BAN khi xu ly lenh quan tri.
    # Nhờ vậy Admin vẫn có thể dùng /mo_ban để tự mở BAN hoặc mở BAN cho user khác.
    is_admin = _is_bot_admin(chat_id)
    ban = _bot_banned_message(chat_id)
    ...
    if upper_cmd.startswith("/MO_BAN"):
        ...
        try:
            send_message(int(target_id), "✅ Bạn đã được MỞ BAN.\n🔓 Có thể sử dụng bot trở lại.", home_markup(int(target_id)))
        except Exception:
            pass
        return
    ...
    if cmd == "🧰 CHỌN TOOL":
        ...
        if not valid or not rec:
            if _is_bot_admin(chat_id):
                send_message(chat_id, "🧰 CHỌN TOOL\n\nHãy chọn tool muốn sử dụng:", tool_menu_markup())
                return
    ...
    if cmd in tool_buttons:
        # Sau restart/redeploy, đồng bộ quyền theo Telegram ID trước khi chạy tool.
        if not _is_bot_admin(chat_id):
            _restore_bot_auth_from_server(chat_id)
    ...
    if cmd in ("/start", "/menu", "/tools"):
        # Ưu tiên khôi phục Key ACTIVE theo Telegram ID từ Key Server.
        # Vì vậy restart/redeploy Render không bắt người dùng nhập lại Key.
        if not _is_bot_admin(chat_id):
            server_replied, server_rec = _restore_bot_auth_from_server(chat_id)
            if server_replied:
                if server_rec:
                    send_message(chat_id, _auth_menu_text(chat_id), home_markup())
                else:
                    # Server đã xác nhận không còn Key ACTIVE. Xóa local auth cũ
                    # để không vô tình cho dùng bằng quyền đã hết/thu hồi.
                    with BOT_AUTH_LOCK:
        ...
        # Fallback: nếu Key Server tạm thời không truy cập được thì dùng auth local.
        valid, rec = _bot_auth_valid(chat_id)
        if valid and rec:
            send_message(chat_id, _auth_menu_text(chat_id), home_markup())
        else:
    ...
    # ADMIN: /revoke <telegram_id> [FREE|VIP]
    if cmd.lower().startswith(("/revoke", "/revoke_key")):
        ...
        if requested_type in (None, "VIP"):
            server_ok, server_msg = _revoke_vip_on_server(target_id)
            # Nếu yêu cầu VIP và server từ chối thì không báo thu hồi thành công.
    ...
    # ADMIN: /ban <telegram_id> [hours] — khóa người dùng thủ công
    ...
    if cmd.lower().startswith("/login"):
        ...
        # Lưu/đồng bộ Key FREE lên Key Server trước để Telegram ID được ghi bền vững.
        registered, reg_data = _register_free_key_on_server(chat_id, user_key, _bot_device_id(chat_id))
        if not registered:
            send_message(
                chat_id,
                "❌ Không thể lưu Key FREE lên Key Server.\n\n"
                "Vui lòng kiểm tra KEY_SERVER_URL và KEY_SERVER_ADMIN_SECRET trên Render rồi thử lại.",
                _auth_menu_markup(),
            )
            return

        verified, verify_data = _verify_free_key_on_server(chat_id, user_key)
        if not verified:
            status = str((verify_data or {}).get("status", "INVALID")).upper()
            if status == "EXPIRED":
                msg = "❌ KEY FREE ĐÃ HẾT HẠN."
            elif status == "TELEGRAM_ACTIVE_KEY":
                msg = "⚠️ Telegram ID này đã có Key FREE còn hiệu lực."
            elif status == "TELEGRAM_MISMATCH":
                msg = "❌ KEY FREE ĐÃ ĐƯỢC LIÊN KẾT VỚI TELEGRAM ID KHÁC."
            else:
                msg = "❌ Không thể xác thực Key FREE trên Key Server."
            send_message(chat_id, msg, _auth_menu_markup())
            return

        # Một link FREE = một KEY FREE = chỉ 1 lần kích hoạt.
        _mark_free_key_used(user_key, chat_id, pending.get("link"))
        server_expires = _parse_iso_ts((verify_data or {}).get("expires_at"))
        expires = server_expires or (_now_ts() + 24 * 3600)
...
def _verify_bot_vip_key(chat_id, user_key):
    # Luồng Key VIP:
    # - Mỗi KEY VIP trên server chỉ được liên kết với 1 TELEGRAM ID.
    # - Một TELEGRAM ID có thể dùng nhiều KEY VIP khác nhau.
    # - Server phải là nơi quyết định key đã thuộc Telegram ID khác hay chưa.
    ...
    try:
        ...
        # Không khóa theo Telegram ID nữa.
        # Quy tắc: MỖI KEY VIP trên server chỉ được liên kết với 1 TELEGRAM ID.
        # Một TELEGRAM ID có thể kích hoạt các KEY VIP khác nhau.
        ...
        send_message(chat_id,
                     ...
                     "Bạn có thể sử dụng bot và logic VIP được cấp quyền.",
                     home_markup())
    except Exception as exc:
...
def _handle_free_key_request(chat_id):
    ...
    # Không tạo link mới nếu Telegram ID đã có FREE còn hạn trên Key Server.
    if not _is_bot_admin(chat_id):
        server_replied, server_rec = _restore_bot_auth_from_server(chat_id)
        if server_replied and server_rec:
            send_message(chat_id, "✅ Bạn đang có Key FREE còn hiệu lực.\n\nBạn có thể sử dụng bot bình thường.", home_markup())
            return
    ...
    device_id = _bot_device_id(chat_id)
    send_message(chat_id, "⏳ ĐANG TẠO LINK LẤY KEY FREE...\nVui lòng chờ hệ thống tạo link.")
    try:
        key = secrets.token_hex(4)
        telegraph_url = create_telegraph_page(key, device_id)
        if not telegraph_url:
            send_message(chat_id, "❌ KHÔNG THỂ TẠO ĐƯỢC LINK FREE.\n\nVui lòng thử lại sau.")
            return

        final_url = _create_free_short_link(telegraph_url)
        if not final_url:
            raise RuntimeError("Không tạo được URL lấy Key FREE")
...
def run_worker(tool_key):
    ...
    # Worker riêng cho từng tool: một lỗi runtime của tool không làm sập
    # toàn bộ Telegram bot/process chính.
    try:
        fn()
    except KeyboardInterrupt:
        pass
    except SystemExit as exc:
        # Chỉ ADMIN mới nhìn thấy lỗi hệ thống từ worker.
        code = exc.code
        if code not in (None, 0):
            chat_id = os.getenv("VBTOOL_TELEGRAM_ID", "").strip()
            if chat_id and _is_bot_admin(chat_id):
                print(
                    f"❌ {TOOLS.get(str(tool_key).lower(), (str(tool_key),))[0]} "
                    f"kết thúc với mã lỗi: {code}",
                    flush=True,
                )
    except Exception as exc:
        # Lỗi runtime nội bộ không gửi cho người dùng thường; chỉ ADMIN được thấy.
        chat_id = os.getenv("VBTOOL_TELEGRAM_ID", "").strip()
        if chat_id and _is_bot_admin(chat_id):
            tool_name = TOOLS.get(str(tool_key).lower(), (str(tool_key),))[0]
            print(
                f"❌ {tool_name} gặp lỗi runtime: {type(exc).__name__}: {exc}",
                flush=True,
            )
...
class Layout:
    ...
    def split_row(self, *args): self.rows.extend(args)
    def split_column(self, *args): self.rows.extend(args)
    def update(self, value): self.value=value
    def __str__(self): return "\n".join(str(x) for x in getattr(self, 'rows', []))
...
LINK4M_TOKEN = os.getenv("LINK4M_TOKEN", "").strip()
ACCESS_TOKEN = os.getenv("ACCESS_TOKEN", "").strip()
VUOTNHANH_TOKEN = os.getenv("VUOTNHANH_TOKEN", "").strip()

ENCRYPTION_KEY = hashlib.sha256(b"VBTool_VIP").digest()

#========Telegaph================
TELEGRAPH_API = "https://api.telegra.ph"
TELEGRAPH_TOKEN_FILE = str(BOT_DATA_DIR / "telegraph_access_token.txt")

def _load_telegraph_token():
    # Ưu tiên token Render Environment Variables, sau đó dùng token đã lưu.
    token = str(ACCESS_TOKEN or "").strip()
    if token:
        return token
    try:
        p = Path(TELEGRAPH_TOKEN_FILE)
        if p.exists():
...
def _save_telegraph_token(token):
    token = str(token or "").strip()
    if not token:
        return
    try:
        Path(TELEGRAPH_TOKEN_FILE).write_text(token, encoding="utf-8")
    except Exception:
        pass

def _create_telegraph_account():
    # Tự tạo tài khoản Telegraph nếu ACCESS_TOKEN chưa có hoặc token cũ không còn hợp lệ.
    try:
        resp = requests.post(
            f"{TELEGRAPH_API}/createAccount",
            data={
                "short_name": "VBTOOLBOT",
                "author_name": "VBTOOL",
            },
            timeout=15,
        )
        if resp.status_code != 200:
            raise RuntimeError(f"Telegraph createAccount HTTP {resp.status_code}")
        data = resp.json()
        token = str((data.get("result") or {}).get("access_token") or "").strip()
        if not data.get("ok") or not token:
            raise RuntimeError(f"Telegraph createAccount failed: {data.get('error', 'UNKNOWN_ERROR')}")
        _save_telegraph_token(token)
        return token
    except Exception as exc:
        print(f"[Telegraph] createAccount error: {type(exc).__name__}: {exc}", flush=True)
        return ""
...
def create_telegraph_page(key, device_id):
    content = json.dumps([
        ...
        {"tag": "p", "children": ["⚠️ Key chỉ sử dụng trên tài khoản này."]},
        {"tag": "p", "children": ["⚠️ Key chỉ được kích hoạt 1 lần, 1 tài khoản duy nhất."]},
    ])

    token = _load_telegraph_token()

    def _request_create_page(access_token):
        data = {
    ...
    try:
        # 1) Dùng token hiện có.
        if token:
            resp, body = _request_create_page(token)
            if resp.status_code == 200 and body.get("ok"):
                return (body.get("result") or {}).get("url")

        # 2) Token thiếu/hết hạn -> tự tạo token Telegraph mới rồi thử lại.
        ...
        print(f"[Telegraph] createPage failed: HTTP {resp.status_code} - {body.get('error', body)}", flush=True)
        return None
    except Exception as exc:
        print(f"[Telegraph] createPage error: {type(exc).__name__}: {exc}", flush=True)
        return None


def _create_free_short_link(telegraph_url: str) -> str:
    """Tạo chuỗi link FREE: Vượt Nhanh -> Link4m -> Telegraph."""
    telegraph_url = str(telegraph_url or "").strip()
    if not telegraph_url:
        raise RuntimeError("Thiếu Telegraph URL")

    # Bước 1: Telegraph là trang cuối cùng chứa KEY.
    # Bước 2: Link4m bọc bên ngoài Telegraph.
    if not LINK4M_TOKEN:
        raise RuntimeError("LINK4M_TOKEN chưa được cấu hình")

    try:
        r1 = requests.get(
            LINK4M_API,
            params={"api": LINK4M_TOKEN, "url": telegraph_url},
            timeout=15,
        )
        body1 = (r1.text or "").strip()[:500]
        if r1.status_code != 200:
            print(f"[FREE LINK] Link4m lỗi HTTP {r1.status_code} | {body1}", flush=True)
            raise RuntimeError(f"Link4m HTTP {r1.status_code}")

        try:
            data1 = r1.json()
        except ValueError:
            raise RuntimeError("Link4m trả về dữ liệu không hợp lệ")

        if data1.get("status") != "success" or not data1.get("shortenedUrl"):
            raise RuntimeError(f"Link4m tạo link thất bại: {body1}")

        link4m_url = str(data1["shortenedUrl"]).strip()
        print(f"[FREE LINK] Link4m: {link4m_url}", flush=True)
    except Exception as exc:
        print(f"[FREE LINK] Link4m exception: {type(exc).__name__}: {exc}", flush=True)
        raise

    # Bước 3: Vượt Nhanh bọc bên ngoài Link4m.
    if not VUOTNHANH_TOKEN:
        raise RuntimeError("VUOTNHANH_TOKEN chưa được cấu hình")

    try:
        r2 = requests.get(
            VUOTNHANH_API,
            params={"api": VUOTNHANH_TOKEN, "url": link4m_url},
            timeout=15,
        )
        body2 = (r2.text or "").strip()[:500]
        if r2.status_code != 200:
            print(f"[FREE LINK] VuotNhanh lỗi HTTP {r2.status_code} | {body2}", flush=True)
            raise RuntimeError(f"Vượt Nhanh HTTP {r2.status_code}")

        try:
            data2 = r2.json()
        except ValueError:
            raise RuntimeError("Vượt Nhanh trả về dữ liệu không hợp lệ")

        if data2.get("status") != "success" or not data2.get("shortenedUrl"):
            raise RuntimeError(f"Vượt Nhanh tạo link thất bại: {body2}")

        final_url = str(data2["shortenedUrl"]).strip()
        print(f"[FREE LINK] Vượt Nhanh: {final_url}", flush=True)
        return final_url
    except Exception as exc:
        print(f"[FREE LINK] Vượt Nhanh exception: {type(exc).__name__}: {exc}", flush=True)
        raise
...
def create_free_key_link(device_id: str) -> Optional[str]:
    ...
    try:
        # Bắt buộc chuỗi: Vượt Nhanh -> Link4m -> Telegraph.
        final_url = _create_free_short_link(telegraph_url)
        if not final_url:
            console.print("[bold red]❌ KHÔNG THỂ TẠO ĐƯỢC LINK FREE.[/bold red]")
            return None
...
def build_logo_with_gradient(logo_text: str) -> str:
    return logo_text.strip("\n")
...
def show_ultimate_header():
    ...
    if activation:
        try:
            key_type = str(activation.get("key_type", ""))
            expiry_time = datetime.fromisoformat(activation["expiry_time"])
            ...
            if remaining.total_seconds() > 0:
                ...
                key_time = f"⏰ Còn {days} ngày {hours} giờ {minutes} phút" if days else (f"⏰ Còn {hours} giờ {minutes} phút" if hours else f"⏰ Còn {minutes} phút")
            else:
                key_time = "❌ Đã hết hạn"
        except Exception:
    ...
    return (
        build_logo_with_gradient(LOGO_ULTIMATE) + "\n\n"
        f"👤 Admin: Nguyễn Văn Bảo | 📞 Zalo: 0797676482\n"
        f"💎 Loại Key: {key_type or '-'} | {key_time}\n"
        + "━" * 60
    )
    ...
    try:
        ...
        for _ in range(24):
            ...
            if isinstance(new_balance, (int, float)):
                ...
                # Bỏ qua số dư tạm thời sau khi tiền cược vừa bị trừ.
                if is_win:
...
def prompt_settings() -> bool:
    ...
    if os.getenv("VBTOOL_TELEGRAM_MODE") == "1":
        ...
        if selected_key == "ALL":
            pt_str = Prompt.ask("🎯 Mục tiêu lãi (nhập 0 để tắt)").strip()
...
def _cdtd_telegram_capital_summary(game, balance=None):
    """In cấu hình quản lý vốn bằng text để Telegram luôn nhận được.
    Rich Live/screen không phải lúc nào cũng truyền nguyên giao diện động qua Telegram,
    vì vậy các thông tin quan trọng được in thêm thành dòng text rõ ràng.
    """
    ...
    auto = bool(getattr(game, "auto_bet", True))
    nv = max(1, int(getattr(game, "so_nv", 1) or 1))
    current = float(getattr(game, "_telegram_current_bet", base) or base)

    lines = [
        "📊 QUẢN LÝ VỐN",
        f"💰 Cược gốc mỗi ván: {_fmt_cdtd_number(base)} {coin}",
        f"🎯 Cược hiện tại/ván: {_fmt_cdtd_number(current)} {coin}",
        f"👥 Số người đặt: {nv}",
        f"💵 Cược mỗi người: {_fmt_cdtd_number(current / nv)} {coin}",
        f"📈 Gấp thếp sau thua: x{mult:g}",
        f"⏸️ Nghỉ sau thua: {pause} ván" if pause > 0 else "⏸️ Nghỉ sau thua: Tắt",
        f"🛡️ Chống soi: nghỉ 1 ván sau {anti} ván" if anti > 0 else "🛡️ Chống soi: Tắt",
        f"🎯 Mục tiêu số dư: >= {_fmt_cdtd_number(target)} {coin}" if target > 0 else "🎯 Mục tiêu số dư: Tắt",
        f"🛑 Cắt lỗ số dư: <= {_fmt_cdtd_number(stop)} {coin}" if stop > 0 else "🛑 Cắt lỗ số dư: Tắt",
        f"🤖 Auto cược: {'BẬT' if auto else 'TẮT'}",
    ]
    if balance is not None:
        lines.insert(1, f"💳 Số dư hiện tại: {_fmt_cdtd_number(balance)} {coin}")
    cprints("bright_cyan", "\n".join(lines))
...
def main_cdtd():
    ...
    cprints("dim" if "dim" in VBTOOL_COLORS else "white", "   Nhập link ở tin nhắn tiếp theo; có thể thêm nhiều tài khoản.")
...
# ================== PHẦN 5: MAIN ==================
def main():
    # Bot-only: không chạy menu terminal/Rich.
    return bot_main()

if __name__ == "__main__":
    if len(sys.argv) >= 3 and sys.argv[1] == "--worker":
        # Worker Telegram chạy riêng, không chiếm PORT của Web Service cha.
        run_worker(str(sys.argv[2]).strip().lower())
    else:
        # Mở cổng HTTP trước khi vào vòng polling Telegram để Render phát hiện port.
