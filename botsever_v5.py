...
# Chỉ lấy token từ Environment Variables. Không hard-code token trong source.
...
class TelegramConflictError(RuntimeError):
    """409 từ Telegram: đang có một poller khác dùng cùng Bot API token."""
    def __init__(self, method, description=""):
        self.method = str(method or "")
        self.description = str(description or "")
        super().__init__(f"Telegram 409 Conflict ({self.method}): {self.description}")
...
def tg(method, payload=None, timeout=35):
    ...
    # Telegram trả HTTP 409 khi một poller khác đang dùng getUpdates.
    # Không để lỗi này bị biến thành traceback gửi lặp cho Admin.
    if r.status_code == 409:
        raise TelegramConflictError(method, data.get("description", r.text[:500]))
    ...
    def _flusher(self):
        ...
        def _send_part(part):
            ...
            if _looks_like_internal_error(part):
                _notify_admin_code_error(
                    "Output worker chứa lỗi kỹ thuật",
                    chat_id=self.chat_id,
                    tool_key=self.key,
                    traceback_text=part,
                )
                self._enqueue_message(
                    "⚠️ Công cụ gặp lỗi kỹ thuật tạm thời. Vui lòng thử lại; Admin đã được thông báo."
                )
                return
        ...
        if not STOP_ALL.is_set():
            ...
            try:
                if abnormal:
                    send_message(self.chat_id, f"⚠️ {self.name} gặp sự cố và đã tạm dừng. Admin đã nhận thông tin lỗi.")
                else:
...
def bot_main():
    ...
    # Polling không thể chạy đồng thời với webhook. Xóa webhook trước khi bắt đầu.
    try:
        tg("deleteWebhook", {"drop_pending_updates": False}, timeout=20)
    except Exception as exc:
        # Lỗi ở bước dọn webhook không được làm bot chết ngay.
        try:
            _notify_admin_code_error("Telegram deleteWebhook lỗi", exc, tool_key="SYSTEM")
        except Exception:
            pass
    ...
    try:
        while not STOP_ALL.is_set():
            try:
                ...
                # 409 thường xảy ra khi Render đang overlap 2 process/instance
                # hoặc còn một process cũ chưa thoát. Chờ dài thay vì spam lỗi.
                if now - last_conflict_notice >= 60:
                    last_conflict_notice = now
                    _notify_admin_code_error(
                        "Telegram polling đang bị xung đột 409",
                        exc,
                        tool_key="SYSTEM",
                    )

                # Backoff tăng dần, tối đa 30 giây. Khi poller cũ biến mất,
                # process hiện tại tự tiếp tục mà không cần restart toàn bot.
                delay = min(30, 5 + conflict_count * 2)
                time.sleep(delay)
            except Exception as exc:
...
def run_worker(tool_key):
    ...
    if chat_id:
        ...
        # Các biến này được khai báo ở phần engine phía dưới nhưng đến lúc
        # run_worker() được gọi thì module đã nạp xong toàn bộ.
    ...
    try:
        ...
        if code not in (None, 0):
            chat_id = os.getenv("VBTOOL_TELEGRAM_ID", "").strip()
            _notify_admin_code_error(
                "Worker kết thúc bất thường (SystemExit)",
                exc,
                chat_id=chat_id,
                tool_key=tool_key,
            )
        # Không in traceback/chi tiết lỗi ra stdout vì stdout được chuyển về Telegram user.
        raise
    except Exception as exc:
        # Lỗi runtime CHỈ gửi chi tiết cho Admin; user thường không nhận exception/traceback.
        chat_id = os.getenv("VBTOOL_TELEGRAM_ID", "").strip()
        _notify_admin_code_error(
            "Worker gặp lỗi runtime chưa được xử lý",
            exc,
            chat_id=chat_id,
            tool_key=tool_key,
        )
        # Thoát có mã lỗi nhưng không để Python tự in traceback ra stdout/stderr.
        raise SystemExit(1)
...
class _HiloBotLive:
    """Thay Rich Live bằng output dạng tin nhắn khi HILO chạy trong Telegram."""
...
def main_lotto():
    ...
    if bot_mode:
        acc_uid = (selected_account.get("user-id") or selected_account.get("userId") or selected_account.get("uid") or "—")
        print(f"✅ Đăng nhập LOTTO thành công\n👤 userId={acc_uid}", flush=True)
...
def xw_main() -> None:
    ...
    bot_mode = os.getenv("VBTOOL_TELEGRAM_MODE") == "1"
    if bot_mode:
        xw_rgb_print(0, 255, 255, "🎯 CANH CODE XWORLD\n\n🚀 Tool chạy trực tiếp trên Telegram.")
