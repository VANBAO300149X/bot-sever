...
def _vip_store_markup(stock):
    ...
    for pid, plan in KEY_VIP_STORE_PLANS.items():
        ...
        hot = " 🔥" if plan.get("hot") else ""
        disabled = available is not None and int(available) <= 0
        label = f"{plan['name']}: {count_text} key" + (" — HẾT" if disabled else "") + hot
        rows.append([{"text": label, "callback_data": f"vipbuy:{pid}"}])
...
def _vip_store_text(stock=None):
    ...
    for pid, plan in KEY_VIP_STORE_PLANS.items():
        available = stock.get(pid)
        count_text = "Đang cập nhật" if available is None else f"{max(0, int(available))} key"
        hot = " (🔥Bán chạy)" if plan.get("hot") else ""
        lines.append(f"{plan['name']}: {plan['price']:,} VNĐ{hot} — Còn {count_text} trên server".replace(",", "."))
        lines.append("")
    lines.append("📌 Tồn kho được đọc trực tiếp từ server, không tính key đã cấp/đã kích hoạt.")
    lines.append("👉 Chọn gói bên dưới để thanh toán bằng MOMO.")
    return "\n".join(lines).strip()
