...
BOT_AUTH_FILE = "telegram_auth.json"
BOT_VIP_ACTIVATED_FILE = "telegram_vip_activated.json"  # Legacy: không còn dùng để khóa theo Telegram ID
BOT_BAN_FILE = "telegram_bans.json"
...
def handle_message(msg):
    ...
    if cmd in ("/start", "/menu", "/tools"):
        # Chưa xác thực: chỉ hiện 2 nút NHẬP KEY FREE/VIP.
        # Đã xác thực: mới hiện Reply Keyboard chính của bot.
        valid, rec = _bot_auth_valid(chat_id)
        if valid and rec:
            send_message(chat_id, _auth_menu_text(chat_id), home_markup())
        else:
