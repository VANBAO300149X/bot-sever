...
BOT_AUTH_FILE = _data_path("telegram_auth.json")
BOT_VIP_ACTIVATED_FILE = _data_path("telegram_vip_activated.json")  # Legacy
BOT_BAN_FILE = _data_path("telegram_bans.json")
    ...
    try:
        while not STOP_ALL.is_set():
            try:
                ...
                err_text = str(exc)
                if "Telegram HTTP 409" in err_text or ("Conflict" in err_text and "getUpdates" in err_text):
                    print(
                        "[POLL 409] BOT_TOKEN đang được tiến trình khác dùng getUpdates. "
                        "Chỉ chạy 1 worker với token này; đang chờ rồi thử lại...",
...
DEVICE_ID_FILE = _data_path("device_id.txt")
SALT = "VBTOOLKEY"
TEMP_KEY_FILE = _data_path(".temp_key.txt")
ACTIVATION_FILE = _data_path("activation.dat")
LINK_HISTORY_FILE = _data_path("link_history.json")
