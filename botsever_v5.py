...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8344831081:AAGDH8ws84LCzuXKqtRkvZE3-6-0N3K1C78").strip()
POLL_TIMEOUT = 25
MAX_TEXT = 3900
FLUSH_INTERVAL = 0.40
...
def _handle_free_key_request(chat_id):
    ...
    # Nếu đã có link còn hiệu lực thì trả link cũ ngay, không gọi lại API.
    now = _now_ts()
    pending = _bot_pending_record(chat_id)
    if pending.get("mode") == "free" and pending.get("link"):
        if now - float(pending.get("created_at", 0) or 0) < 300:
            send_message(
                chat_id,
    ...
    try:
        ...
        # BƯỚC 1: Telegraph. Có timeout để không bị treo vô hạn.
        telegraph_url = create_telegraph_page(key, device_id)
        if not telegraph_url:
            send_message(chat_id, "❌ Telegraph đang chậm hoặc lỗi. Vui lòng bấm lấy link lại sau ít giây.")
            return

        session = _get_link_http_session()
        request_timeout = (LINK_CREATE_CONNECT_TIMEOUT, LINK_CREATE_READ_TIMEOUT)
        ...
        data1 = _safe_json_response(r1, "Link4m")
        ...
        # BƯỚC 3: Vượt Nhanh. Nếu Vượt Nhanh chậm/lỗi thì dùng Link4m làm fallback
        # để người dùng vẫn có link thay vì chờ thêm hoặc thất bại hoàn toàn.
        ...
        try:
            ...
            data2 = _safe_json_response(r2, "Vuotnhanh")
            if data2.get("status") == "success":
                candidate = str(data2.get("shortenedUrl") or "").strip()
                if candidate:
                    final_url = candidate
        except Exception as exc:
            # Không chặn toàn bộ quá trình chỉ vì Vượt Nhanh lỗi.
            log_debug(f"Vuotnhanh fallback Link4m: {type(exc).__name__}: {exc}")
        ...
        _admin_only_exception_log(chat_id, "Timeout tạo link FREE", "Timeout khi gọi dịch vụ tạo link")
        send_message(chat_id, "❌ Dịch vụ tạo link đang phản hồi chậm. Vui lòng thử lại sau ít giây.")
    except requests.exceptions.RequestException as exc:
        _admin_only_exception_log(chat_id, "Lỗi mạng tạo link FREE", exc)
        send_message(chat_id, "❌ Dịch vụ tạo link đang lỗi kết nối. Vui lòng thử lại sau ít giây.")
    except Exception as exc:
...
#========Telegaph================
# Tạo link nhanh và có timeout rõ ràng để API lỗi/chậm không làm treo bot.
_LINK_HTTP_LOCAL = threading.local()
LINK_CREATE_CONNECT_TIMEOUT = 3.5
LINK_CREATE_READ_TIMEOUT = 7.0


def _get_link_http_session():
    """Session riêng cho từng worker/thread để tái sử dụng kết nối HTTP an toàn."""
    session = getattr(_LINK_HTTP_LOCAL, "session", None)
    if session is None:
        ...
        session.headers.update({
            "User-Agent": "VBTOOL/1.0",
            "Accept": "application/json, text/plain, */*",
            "Connection": "keep-alive",
        })
        _LINK_HTTP_LOCAL.session = session
    return session


def _safe_json_response(response, endpoint_name):
    if response.status_code != 200:
        raise RuntimeError(f"{endpoint_name} HTTP {response.status_code}")
    try:
        data = response.json()
    except ValueError as exc:
        raise RuntimeError(f"{endpoint_name} trả về JSON không hợp lệ") from exc
    if not isinstance(data, dict):
        raise RuntimeError(f"{endpoint_name} trả về dữ liệu không hợp lệ")
    return data
...
def create_telegraph_page(key, device_id):
    ...
    try:
        r = _get_link_http_session().post(
            url,
            data=data,
            timeout=(LINK_CREATE_CONNECT_TIMEOUT, LINK_CREATE_READ_TIMEOUT),
        )
        result = _safe_json_response(r, "Telegraph")
        if result.get("ok"):
...
def is_vip_activated() -> bool:
    """Kiểm tra quyền VIP theo đúng hồ sơ kích hoạt hiện tại.

    Trong Telegram worker, check_activation_valid() lấy quyền trực tiếp
    từ _WORKER_CHAT_ID/BOT_AUTH của từng người dùng, nên VTH phải dùng
    kết quả này thay vì đọc một biến môi trường không tồn tại.
    """
    try:
        is_valid, _, is_vip = check_activation_valid(exit_on_expired=False)
        return bool(is_valid and is_vip)
    except Exception:
        return False
