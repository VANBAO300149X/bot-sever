...
def stop_tool(chat_id, key=None):
    ...
    # Không truyền key => BẮT BUỘC mở menu để người dùng chọn tool cần dừng.
    # Tuyệt đối không tự dừng tool ACTIVE, để tránh dừng nhầm khi nhiều tool chạy song song.
    if key is None:
        running_names = "\n".join(
        ...
        send_message(
            ...
            "🟢 Các tool đang chạy:\n"
            f"{running_names}\n\n"
            "👉 Hãy chọn đúng tool bạn muốn dừng:",
            stop_menu_markup(),
        )
        return
...
def show_status(chat_id):
    ...
    lines = ["🟢 TOOL ĐANG CHẠY", ""]
    for session in sessions:
        lines.append(f" {session.name}")

    # Chỉ hiển thị danh sách tool đang chạy; không có nút Dừng/Menu.
