...
def _purchase_markup():
    return {
        "inline_keyboard": [
            [{"text": "3 NGÀY • 5.000đ", "callback_data": "buy:3d"},
             {"text": "1 TUẦN • 10.000đ", "callback_data": "buy:1w"}],
            [{"text": "2 TUẦN • 20.000đ", "callback_data": "buy:2w"},
             {"text": "1 THÁNG • 30.000đ", "callback_data": "buy:1m"}],
            [{"text": "2 THÁNG • 50.000đ", "callback_data": "buy:2m"}],
            [{"text": "VĨNH VIỄN • 150.000đ", "callback_data": "buy:forever"}],
        ]
    }
...
def _purchase_text():
    return (
        ...
        "🟢 1 THÁNG      • 30.000 VNĐ\n"
        "🟢 2 THÁNG      • 50.000 VNĐ\n"
        "👑 VĨNH VIỄN     • 150.000 VNĐ\n\n"
        "👉 Bấm gói bên dưới để tạo đơn."
    )
...
def _issue_purchase_order(order_id, admin_chat_id):
    ...
    # Kích hoạt Key VIP trực tiếp cho Telegram ID bằng endpoint của app_keys.py.
    try:
        verify_headers = {}
        if KEY_SERVER_ADMIN_SECRET:
            verify_headers["X-Server-Secret"] = KEY_SERVER_ADMIN_SECRET
        verify_resp = requests.post(
            f"{KEY_SERVER_URL}/api/telegram/verify_key",
            json={
                "key": key,
    ...
    if not isinstance(verify_data, dict) or not verify_data.get("success"):
        msg = str(
            verify_data.get("message")
            if isinstance(verify_data, dict)
            else ""
        ) or verify_error or "Không thể kích hoạt Key cho Telegram."
        with BOT_ORDER_LOCK:
            if order_id in BOT_ORDERS:
                BOT_ORDERS[order_id]["status"] = "PROCESSING"
                BOT_ORDERS[order_id]["key"] = key
                BOT_ORDERS[order_id]["error"] = msg
                _save_bot_orders()
        return False, f"Đã lấy Key nhưng kích hoạt Telegram thất bại: {msg}"

    expires_at = verify_data.get("expires_at")
    key_type = verify_data.get("key_type", "VIP")
    with BOT_ORDER_LOCK:
        if order_id in BOT_ORDERS:
            BOT_ORDERS[order_id]["status"] = "COMPLETED"
            BOT_ORDERS[order_id]["key"] = key
            BOT_ORDERS[order_id]["key_type"] = key_type
            BOT_ORDERS[order_id]["expires_at"] = expires_at
            BOT_ORDERS[order_id]["completed_at"] = datetime.now().isoformat()
            _save_bot_orders()

    # Đồng bộ quyền bot ngay lập tức.
    try:
        expiry = _parse_iso_ts(expires_at)
        if expiry is None and pkg["is_forever"]:
            expiry = _now_ts() + 365 * 24 * 3600
        if expiry is not None:
            _set_bot_auth(
                ...
                str(key_type or "VIP"),
                expiry,
                _bot_device_id(int(order["telegram_id"])),
                pkg["duration_hours"],
            )
    except Exception:
        pass
