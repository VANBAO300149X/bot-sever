...
class ToolSession:
    ...
    def _flusher(self):
        ...
        if not STOP_ALL.is_set():
            ...
            try:
                if abnormal:
                    send_message(self.chat_id, ("⚠️ Công cụ gặp sự cố và đã tạm dừng. Admin đã nhận thông tin lỗi." if self.key == "lotto" else f"⚠️ {self.name} gặp sự cố và đã tạm dừng. Admin đã nhận thông tin lỗi."))
                else:
