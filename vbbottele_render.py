...
# Render mặc định dùng filesystem tạm thời. Khi gắn Persistent Disk,
# đặt mount path là /var/data để các file quyền/key sống qua restart/deploy.
#
# Có thể đổi thư mục lưu bằng biến môi trường VBTOOL_DATA_DIR.
if Path("/var/data").is_dir():
    _DEFAULT_DATA_DIR = "/var/data"
else:
    _DEFAULT_DATA_DIR = "."
...
try:
    ...
    # Fallback an toàn khi môi trường không cho tạo /var/data.
    BOT_DATA_DIR = Path(".")
    BOT_DATA_DIR.mkdir(parents=True, exist_ok=True)
...
try:
    print(f"[KEY STORAGE] data_dir={BOT_DATA_DIR}", flush=True)
    print(f"[KEY STORAGE] auth_file={BOT_AUTH_FILE}", flush=True)
except Exception:
    pass
...
def _bot_auth_record(chat_id):
    """Lấy quyền theo Telegram ID và chỉ xóa khi key thực sự hết hạn."""
    ...
    with BOT_AUTH_LOCK:
        ...
        if expiry and _now_ts() >= expiry:
            # Key hết hạn -> xóa hồ sơ ID này.
            BOT_AUTH.pop(target, None)
            _save_json_file(BOT_AUTH_FILE, BOT_AUTH)
            return None

        # Key còn hạn -> giữ nguyên qua mọi lần restart.
        return dict(rec)
        ...
        while not self.stop_event.is_set() or not self.q.empty():
            ...
            if buf and (
                ...
                for part in chunks(text):
                    ...
                    if self.key == "cdtd" and "🚀 ĐANG KHỞI ĐỘNG CDTD..." in part:
...
def start_tool(chat_id, key):
    ...
    send_message(
        ...
        "",
        {"inline_keyboard": [
            [{"text": "🛑 DỪNG TOOL", "callback_data": "stop"}],
...
# Dùng cùng thư mục dữ liệu persistent trên Render để key/ID không mất
# sau khi service restart hoặc deploy.
