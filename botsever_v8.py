...
class ToolSession:
    def __init__(self, chat_id, key):
        ...
        self.auth_monitor_stop_event = threading.Event()
        self.send_queue = queue.Queue(maxsize=200)
        self._last_sent_text = ""
        self._last_sent_ts = 0.0
        self._stopped_due_to_key_expiry = False
        auth_snapshot = _bot_auth_record(chat_id)
        self.auth_expires_at = float(auth_snapshot.get("expires_at", 0) or 0) if auth_snapshot else 0.0
        self.auth_key_type = str(auth_snapshot.get("key_type", "FREE")).upper() if auth_snapshot else "FREE"
    ...
    def start(self):
        ...
        if not _is_bot_admin(self.chat_id) and self.auth_expires_at:
            self.auth_monitor_thread = threading.Thread(
                target=self._auth_expiry_monitor,
                daemon=True,
                name=f"tg-key-monitor-{self.chat_id}-{self.key}",
            )
            self.auth_monitor_thread.start()

    def _auth_expiry_monitor(self):
        """Tự động dừng tool và trả người dùng về menu nhập Key khi hết hạn."""
        try:
            while not self.stop_event.is_set() and not STOP_ALL.is_set():
                if self.auth_monitor_stop_event.wait(1.0):
                    return
                if self.auth_expires_at and time.time() >= self.auth_expires_at:
                    self._stopped_due_to_key_expiry = True
                    self._expected_stop = True
                    try:
                        # Xóa các tin nhắn tool đang chờ gửi để không gửi log cũ
                        # sau khi thông báo Key hết hạn đã xuất hiện.
                        while True:
                    ...
                    try:
                        with BOT_AUTH_LOCK:
                            BOT_AUTH.pop(str(self.chat_id), None)
                            _save_json_file(BOT_AUTH_FILE, BOT_AUTH)
                    except Exception:
                        pass
                    BOT_PENDING.pop(str(self.chat_id), None)
                    _save_bot_pending()
                    try:
                        send_message(
                            self.chat_id,
                        ...
                        send_message(
                            self.chat_id,
                            "🔐 Vui lòng nhập Key FREE hoặc Key VIP mới để tiếp tục.",
                            _auth_menu_markup(),
                        )
                    except Exception as exc:
                        _notify_admin_code_error(
                            "Gửi thông báo Key hết hạn lỗi",
                            exc,
    ...
    def _flusher(self):
        ...
        def _send_part(part):
            if self._stopped_due_to_key_expiry:
                return
        ...
        if not STOP_ALL.is_set() and not self._stopped_due_to_key_expiry:
    ...
    def stop(self):
        ...
        self._expected_stop = True
        self.auth_monitor_stop_event.set()
        self.sender_stop_event.set()
        proc = self.proc
        if not proc:
            self.stop_event.set()
...
def start_tool(chat_id, key):
    # Đã chọn tool thì không còn ở trạng thái chờ nhập Key VIP/FREE.
    # Xóa pending cũ để link game hoặc input của tool không bị bắt nhầm
    # vào luồng xác thực Key.
...
def handle_message(msg):
    ...
    # Nếu một tool đang chạy thì tin nhắn phải được chuyển cho tool trước,
    # không được coi link/game input là Key VIP do pending cũ còn sót.
    session = get_session(chat_id)
    pending = _bot_pending_record(chat_id)
...
def _verify_bot_vip_key(chat_id, user_key):
    ...
    try:
        ...
        # Xác thực thành công = thoát hoàn toàn trạng thái chờ nhập Key VIP.
        BOT_PENDING.pop(str(chat_id), None)
        _save_bot_pending()
        remaining = max(0, rec["expires_at"] - _now_ts())
        send_message(chat_id,
