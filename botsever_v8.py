...
class _WorkerSafeOutput:
    """Bảo vệ stdout/stderr của Telegram worker khi pipe phía cha đã đóng.

    stdout của worker được ToolSession giữ qua subprocess.PIPE. Nếu session bị đóng
    hoặc tiến trình cha không còn đọc pipe, Python sẽ ném BrokenPipeError tại print().
    Khi đó worker không nên auto-restart vòng lặp vô hạn; thoát bình thường là an toàn.
    """
    ...
    def write(self, data):
        try:
            ...
            # Pipe IPC đã mất: coi đây là một lần dừng bình thường, không restart.
...
BOT_VIP_ORDERS_FILE = str(BOT_DATA_DIR / "telegram_vip_orders.json")
...
BOT_VIP_ORDERS = _load_json_file(BOT_VIP_ORDERS_FILE, {})
BOT_VIP_ORDERS_LOCK = threading.RLock()

def _save_vip_orders():
    return _save_json_file(BOT_VIP_ORDERS_FILE, BOT_VIP_ORDERS)
...
def send_photo(chat_id, photo_path, caption=None, markup=None):
    """Gửi ảnh lên Telegram bằng multipart/form-data."""
    if not BOT_TOKEN:
        return None
    try:
        ...
        if os.getenv("VBTOOL_TELEGRAM_MODE") == "1":
            try:
                with (BOT_DATA_DIR / "bot_runtime_errors.log").open("a", encoding="utf-8") as f:
                    f.write(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [Telegram sendPhoto] {type(exc).__name__}: {exc}\n")
            except Exception:
                pass
        return None
...
def handle_message(msg):
    ...
    # MUA KEY VIP: chức năng công khai, không yêu cầu đã xác thực key.
    if cmd == "🛒 MUA KEY VIP":
        BOT_PENDING.pop(str(chat_id), None)
        _save_bot_pending()
        show_vip_store(chat_id)
        return
    ...
    if cmd.lower().startswith("/login"):
        # /login luôn chuyển hẳn sang FREE, hủy pending VIP trước khi đọc Key.
        BOT_PENDING.pop(str(chat_id), None)
        _save_bot_pending()
        _reset_vip_spam(chat_id)
        parts = cmd.split(maxsplit=1)
        ...
        if user_key != expected_key:
            # Key FREE sai chỉ báo lỗi, tuyệt đối không cộng vào bộ đếm BAN VIP.
            send_message(
                chat_id,
                "❌ Key FREE không đúng.\n\n"
...
def handle_callback(query):
    ...
    if data == "vipstore:refresh":
        message_id = ((query.get("message") or {}).get("message_id"))
        show_vip_store(chat_id, refresh=True, message_id=message_id)
    elif data == "vipstore:open":
        show_vip_store(chat_id)
    elif data.startswith("vipbuy:"):
        create_and_show_vip_payment(chat_id, data.split(":", 1)[1])
    elif data.startswith("viprequest:"):
        request_vip_admin_approval(chat_id, data.split(":", 1)[1])
    elif data.startswith("vipcancel:"):
        cancel_vip_order(chat_id, data.split(":", 1)[1])
    elif data.startswith("vipapprove:"):
        approve_vip_order(chat_id, data.split(":", 1)[1])
    elif data.startswith("vipreject:"):
        reject_vip_order(chat_id, data.split(":", 1)[1])
    elif data == "home":
        with VIP_STORE_VIEWS_LOCK:
            VIP_STORE_VIEWS.pop(str(chat_id), None)
        valid, rec = _bot_auth_valid(chat_id)
        if valid and rec:
            send_message(chat_id, _auth_menu_text(chat_id), home_markup())
        else:
...
# ================== CỬA HÀNG KEY VIP ==================
# API tồn kho / mua key được tách khỏi API verify/revoke.
# Có thể đổi endpoint bằng biến môi trường mà không cần sửa code.
KEY_STORE_STOCK_PATH = os.getenv("KEY_STORE_STOCK_PATH", "/api/key_counts").strip() or "/api/key_counts"
KEY_STORE_BUY_PATH = os.getenv("KEY_STORE_BUY_PATH", "/api/issue_key").strip() or "/api/issue_key"
KEY_STORE_API_TOKEN = os.getenv("KEY_STORE_API_TOKEN", "").strip()

KEY_VIP_STORE_PLANS = {
    "3d": {"name": "3 NGÀY", "price": 5000, "duration_hours": 72},
    "7d": {"name": "1 TUẦN", "price": 10000, "duration_hours": 168, "hot": True},
    "14d": {"name": "2 TUẦN", "price": 20000, "duration_hours": 336},
    "1m": {"name": "1 THÁNG", "price": 30000, "duration_hours": 720},
    "2m": {"name": "2 THÁNG", "price": 50000, "duration_hours": 1440},
}

# QR gốc do chủ bot cung cấp. Bot tự tạo lại QR theo số tiền + nội dung của từng đơn.
MOMO_BASE_QR_PAYLOAD = (
    "00020101021138630010A000000727013300069710250119PSP2611614000000227"
    "0208QRIBFTTA53037045802VN62200516MOMOW2W1253204726304E205"
)
MOMO_WALLET_NAME = "MOMO"
MOMO_ACCOUNT_NAME = "TRẦN TUẤN HÙNG"

VIP_STORE_VIEWS = {}
VIP_STORE_VIEWS_LOCK = threading.RLock()
VIP_STORE_REFRESH_SECONDS = max(10.0, float(os.getenv("KEY_STORE_REFRESH_SECONDS", "30")))
VIP_STORE_REFRESH_THREAD = None
VIP_STORE_REFRESH_STOP = threading.Event()
VIP_QR_DIR = BOT_DATA_DIR / "vip_qr"
VIP_QR_DIR.mkdir(parents=True, exist_ok=True)


def _key_store_headers():
    headers = {"Accept": "application/json"}
    if KEY_STORE_API_TOKEN:
        headers["Authorization"] = f"Bearer {KEY_STORE_API_TOKEN}"
        headers["X-API-Key"] = KEY_STORE_API_TOKEN
    return headers


def _key_store_stock_from_data(data):
    """Chuẩn hóa /api/key_counts => {plan_id: stock}."""
    counts = data.get("counts") if isinstance(data, dict) else None
    if counts is None and isinstance(data, dict) and isinstance(data.get("data"), dict):
        counts = data["data"].get("counts")
    if not isinstance(counts, dict):
        return {}
    duration_map = {72: "3d", 168: "7d", 336: "14d", 720: "1m", 1440: "2m"}
    result = {}
    for raw_duration, raw_count in counts.items():
        try:
            pid = duration_map.get(int(float(raw_duration)))
            if pid:
                result[pid] = max(0, int(raw_count or 0))
        except (TypeError, ValueError):
            pass
    return result


def _key_store_get_stock():
    try:
        resp = requests.get(f"{KEY_SERVER_URL}/api/key_counts", headers=_key_store_headers(), timeout=12)
        data = resp.json() if resp.content else {}
        if resp.status_code != 200 or not isinstance(data, dict):
            return False, {}, {"message": f"HTTP {resp.status_code}"}
        return True, _key_store_stock_from_data(data), data
...
def _key_store_buy(chat_id, plan_id, order_id=None):
    """Chỉ gọi POST cấp key sau khi admin duyệt. Không retry POST."""
    plan_id = str(plan_id).strip().lower()
    plan = KEY_VIP_STORE_PLANS.get(plan_id)
    if not plan:
        return False, {"message": "Gói key không hợp lệ."}
    payload = {
        "package": str(plan_id),
        "plan": str(plan_id),
        ...
        "telegram_id": str(chat_id),
        "quantity": 1,
        "order_id": str(order_id or f"TG{chat_id}-{int(time.time())}-{secrets.token_hex(5)}"),
        "note": "VBXW-BOT VIP PURCHASE",
    }
    try:
        resp = requests.post(
            f"{KEY_SERVER_URL}/api/issue_key",
            json=payload,
            headers=_key_store_headers(),
            timeout=15,
        )
        data = resp.json() if resp.content else {}
        if resp.status_code != 200 or not isinstance(data, dict):
            return False, {"message": f"HTTP {resp.status_code}", "raw": data}
        if bool(data.get("success", False)):
            return True, data
        return False, data
    except (requests.exceptions.Timeout, requests.exceptions.ConnectionError) as exc:
        return False, {"message": f"Không kết nối được Key Server: {exc}"}
    except Exception as exc:
        return False, {"message": str(exc)}


def _extract_purchased_key(data):
    if not isinstance(data, dict):
        return ""
    vals = [data.get("key"), data.get("vip_key"), data.get("license_key")]
    nested = data.get("data")
    if isinstance(nested, dict):
        vals += [nested.get("key"), nested.get("vip_key"), nested.get("license_key")]
    return next((str(v).strip() for v in vals if v is not None and str(v).strip()), "")
...
def _tlv_parse(text):
    ...
    while i < len(text):
        if i + 4 > len(text):
            raise ValueError("TLV không hợp lệ")
        tag = text[i:i+2]
        length = int(text[i+2:i+4])
        value = text[i+4:i+4+length]
        if len(value) != length:
            raise ValueError("TLV thiếu dữ liệu")
...
def _build_momo_dynamic_payload(amount, content):
    """Thêm số tiền (54) và nội dung (62.05), sau đó tính lại CRC (63)."""
    raw = MOMO_BASE_QR_PAYLOAD
    body = raw[:-8] if raw[-8:-6] == "63" else raw.rsplit("6304", 1)[0]
    items = _tlv_parse(body)
    replaced = []
    has_amount = False
    has_additional = False
    for tag, value in items:
        if tag == "54":
            replaced.append(("54", str(int(amount))))
            has_amount = True
        elif tag == "62":
            try:
                sub = _tlv_parse(value)
                sub2 = []
                has_content = False
                for stag, sval in sub:
                    if stag == "05":
                        sub2.append(("05", str(content)[:99]))
                        has_content = True
                    else:
                        sub2.append((stag, sval))
                if not has_content:
                    sub2.append(("05", str(content)[:99]))
                replaced.append(("62", _tlv_build(sub2)))
            except Exception:
                replaced.append(("62", _tlv_build([("05", str(content)[:99])])))
            has_additional = True
        else:
            replaced.append((tag, value))
        if tag == "53" and not has_amount:
            replaced.append(("54", str(int(amount))))
            has_amount = True
    if not has_amount:
        replaced.append(("54", str(int(amount))))
    if not has_additional:
        replaced.append(("62", _tlv_build([("05", str(content)[:99])])))
    body2 = _tlv_build(replaced) + "6304"
    return body2 + _crc16_ccitt_false(body2)


def _make_dynamic_momo_qr(amount, content, order_id):
    """Tạo QR MoMo động. Ưu tiên thư viện qrcode; nếu server chưa cài thì dùng fallback QuickChart."""
    payload = _build_momo_dynamic_payload(amount, content)
    path = VIP_QR_DIR / f"{re.sub(r'[^A-Za-z0-9_-]', '_', str(order_id))}.png"
    ...
    try:
        ...
        # Render/VPS chưa cài qrcode: dùng requests (đã là dependency bắt buộc)
        # để lấy ảnh QR từ QuickChart, tránh làm bot crash vì ModuleNotFoundError.
        try:
            resp = requests.get(
            ...
            if "image" not in content_type and not resp.content.startswith(b"\x89PNG"):
                raise RuntimeError("QuickChart không trả về ảnh QR hợp lệ")
            path.write_bytes(resp.content)
            return str(path)
        except Exception as exc:
            _notify_admin_code_error("Lỗi tạo QR Momo (fallback)", exc, tool_key="SYSTEM")
            return None
    except Exception as exc:
        _notify_admin_code_error("Lỗi tạo QR Momo", exc, tool_key="SYSTEM")
        return None
...
def _new_payment_content():
    ...
    return f"VB-{letters}{digits}"


def _vip_store_markup(stock):
    rows = []
    for pid, plan in KEY_VIP_STORE_PLANS.items():
        # Nút chỉ hiển thị tên gói; số tồn được hiển thị ở phần nội dung phía trên.
        rows.append([{"text": plan["name"], "callback_data": f"vipbuy:{pid}"}])
    rows.append([{"text": "↩️ QUAY LẠI", "callback_data": "home"}])
    return {"inline_keyboard": rows}


def _vip_store_text(stock=None):
    stock = stock or {}
    lines = ["⚡BẢNG GIÁ KEY VIP VBXW-BOT⚡", ""]
    for pid, plan in KEY_VIP_STORE_PLANS.items():
        available = stock.get(pid)
        count_text = "?" if available is None else str(max(0, int(available)))
        hot = " (🔥Bán chạy)" if plan.get("hot") else ""
        lines.append(f"{plan['name']}: {plan['price']:,} VNĐ{hot} — Còn {count_text}".replace(",", "."))
        lines.append("")
    return "\n".join(lines).strip()


def _edit_vip_store_message(chat_id, message_id, stock):
    try:
        text = _vip_store_text(stock)
        tg("editMessageText", {
            "chat_id": chat_id,
            "message_id": int(message_id),
            "text": text[:4090],
            "disable_web_page_preview": True,
            "reply_markup": _vip_store_markup(stock),
        }, timeout=12)
        return True
    except Exception:
        return False
...
def _start_vip_store_auto_refresh():
    global VIP_STORE_REFRESH_THREAD
    if VIP_STORE_REFRESH_THREAD is not None and VIP_STORE_REFRESH_THREAD.is_alive():
        return
    VIP_STORE_REFRESH_STOP.clear()
    VIP_STORE_REFRESH_THREAD = threading.Thread(target=_vip_store_auto_refresh_loop, daemon=True, name="vip-store-stock-refresh")
    VIP_STORE_REFRESH_THREAD.start()


def _vip_store_auto_refresh_loop():
    while not STOP_ALL.is_set() and not VIP_STORE_REFRESH_STOP.wait(VIP_STORE_REFRESH_SECONDS):
        with VIP_STORE_VIEWS_LOCK:
            views = [(int(k), int(v.get("message_id", 0))) for k, v in VIP_STORE_VIEWS.items() if v.get("message_id")]
        if not views:
            continue
        ok, stock, _ = _key_store_get_stock()
        if ok:
            for chat_id, message_id in views:
                _edit_vip_store_message(chat_id, message_id, stock)


def show_vip_store(chat_id, refresh=False, message_id=None):
    _start_vip_store_auto_refresh()
    ok, stock, meta = _key_store_get_stock()
    if not ok:
        send_message(chat_id, f"⚠️ Không lấy được tồn kho từ Key Server.\n\n{meta.get('message', 'Lỗi kết nối')}")
        return
    if message_id is not None and _edit_vip_store_message(chat_id, message_id, stock):
        return
    sent = send_message(chat_id, _vip_store_text(stock), _vip_store_markup(stock))
    if sent and sent.get("message_id"):
        with VIP_STORE_VIEWS_LOCK:
            VIP_STORE_VIEWS[str(chat_id)] = {"message_id": int(sent["message_id"]), "last_update": _now_ts()}


def _pending_order_for_user(chat_id):
    active = {"AWAITING_PAYMENT", "PENDING_ADMIN", "PROCESSING"}
    with BOT_VIP_ORDERS_LOCK:
        for order in BOT_VIP_ORDERS.values():
            if str(order.get("telegram_id")) == str(chat_id) and str(order.get("status")) in active:
                return dict(order)
    return None


def _create_vip_order(chat_id, plan_id):
    plan = KEY_VIP_STORE_PLANS.get(str(plan_id).strip().lower())
    if not plan:
        return False, None, "Gói key không hợp lệ."
    existing = _pending_order_for_user(chat_id)
    if existing:
        return False, existing, "Bạn đang có một yêu cầu mua KEY VIP đang chờ xử lý."
    content = _new_payment_content()
    order_id = f"VIP-{chat_id}-{int(time.time())}-{secrets.token_hex(4).upper()}"
    qr_path = _make_dynamic_momo_qr(plan["price"], content, order_id)
    if not qr_path:
        return False, None, "Không tạo được QR thanh toán."
    order = {
        "order_id": order_id,
        "telegram_id": str(chat_id),
        "username": "",
    ...
    with BOT_VIP_ORDERS_LOCK:
        BOT_VIP_ORDERS[order_id] = order
        _save_vip_orders()
    return True, order, None


def _user_payment_caption(order):
    return (
        f"📦 GÓI: {order['plan_name']}\n\n"
        f"VÍ: {MOMO_WALLET_NAME}\n"
        f"Tên chủ tài khoản: {MOMO_ACCOUNT_NAME}\n"
        f"SỐ TIỀN: {int(order['amount']):,}\n"
        f"NỘI DUNG: {order['content']}\n\n"
        "⚠️ Lưu ý: sau khi quét QR hãy kiểm tra số tiền và nội dung trước khi chuyển.\n"
        "✅ QR đã cố gắng điền sẵn số tiền + nội dung; tùy phiên bản MOMO, ứng dụng có thể yêu cầu kiểm tra/nhập lại."
    ).replace(",", ".")


def _payment_markup(order_id, status="AWAITING_PAYMENT"):
    if status == "AWAITING_PAYMENT":
        return {"inline_keyboard": [
            [{"text": "📩 GỬI YÊU CẦU DUYỆT CHO ADMIN", "callback_data": f"viprequest:{order_id}"}],
            [{"text": "❌ HỦY ĐƠN", "callback_data": f"vipcancel:{order_id}"}],
        ]}
    return {"inline_keyboard": [[{"text": "↩️ QUAY LẠI MUA KEY", "callback_data": "vipstore:open"}]]}


def create_and_show_vip_payment(chat_id, plan_id):
    ok, stock, _ = _key_store_get_stock()
    if not ok:
        send_message(chat_id, "⚠️ Không kiểm tra được kho Key Server. Vui lòng thử lại sau.")
        return
    if int(stock.get(plan_id, 0)) <= 0:
        show_vip_store(chat_id)
        return
    created, order, error = _create_vip_order(chat_id, plan_id)
    if not created:
        if order:
            send_message(chat_id, f"⚠️ {error}\n\n📦 Gói đang chờ: {order.get('plan_name')}\n🧾 Mã đơn: {order.get('order_id')}")
        else:
            send_message(chat_id, f"❌ {error}")
        return
    result = send_photo(chat_id, order["qr_path"], _user_payment_caption(order), _payment_markup(order["order_id"]))
    if result is None:
        send_message(chat_id, "❌ Không gửi được QR thanh toán. Vui lòng thử lại.")


def _format_admin_order(order):
    username = str(order.get("username") or "").strip()
    user_line = f"👤 Telegram ID: {order.get('telegram_id')}" + (f"\n👤 Username: @{username}" if username else "")
    return (
        "💰 YÊU CẦU DUYỆT MUA KEY VIP\n\n"
        f"{user_line}\n"
        f"📦 Gói: {order.get('plan_name')}\n"
        f"💵 Số tiền: {int(order.get('amount', 0)):,} VNĐ\n"
        f"🧾 Nội dung: {order.get('content')}\n"
        f"🆔 Mã đơn: {order.get('order_id')}\n\n"
        "⚠️ Hãy kiểm tra giao dịch MOMO trước khi duyệt."
    ).replace(",", ".")


def _admin_payment_markup(order_id):
    return {"inline_keyboard": [[
        {"text": "✅ DUYỆT + CẤP KEY", "callback_data": f"vipapprove:{order_id}"},
        {"text": "❌ TỪ CHỐI", "callback_data": f"vipreject:{order_id}"},
    ]]}


def _notify_admin_payment_request(order):
    sent = False
    for admin_id in sorted(ADMIN_CHAT_IDS):
        result = send_message(admin_id, _format_admin_order(order), _admin_payment_markup(order["order_id"]))
        if result is not None:
            sent = True
    return sent


def request_vip_admin_approval(chat_id, order_id):
    order_id = str(order_id)
    with BOT_VIP_ORDERS_LOCK:
        order = BOT_VIP_ORDERS.get(order_id)
        if not isinstance(order, dict) or str(order.get("telegram_id")) != str(chat_id):
            send_message(chat_id, "❌ Đơn mua key không tồn tại hoặc không thuộc tài khoản này.")
            return
        if order.get("status") != "AWAITING_PAYMENT":
            send_message(chat_id, f"ℹ️ Đơn này đang ở trạng thái: {order.get('status')}.")
            return
        order["status"] = "PENDING_ADMIN"
        order["requested_at"] = _now_iso()
        _save_vip_orders()
    if not _notify_admin_payment_request(order):
        with BOT_VIP_ORDERS_LOCK:
            order["status"] = "AWAITING_PAYMENT"
            _save_vip_orders()
        send_message(chat_id, "❌ Không gửi được yêu cầu cho Admin. Vui lòng thử lại.")
        return
    send_message(chat_id, "✅ Đã gửi yêu cầu duyệt cho Admin.\n\n⏳ Vui lòng chờ Admin kiểm tra giao dịch MOMO và duyệt key.")


def cancel_vip_order(chat_id, order_id):
    with BOT_VIP_ORDERS_LOCK:
        order = BOT_VIP_ORDERS.get(str(order_id))
        if not isinstance(order, dict) or str(order.get("telegram_id")) != str(chat_id):
            send_message(chat_id, "❌ Không tìm thấy đơn mua key.")
            return
        if order.get("status") != "AWAITING_PAYMENT":
            send_message(chat_id, "⚠️ Đơn đã gửi duyệt/đang xử lý nên không thể hủy bằng nút này.")
            return
        order["status"] = "CANCELLED"
        order["cancelled_at"] = _now_iso()
        _save_vip_orders()
    send_message(chat_id, "✅ Đã hủy đơn mua KEY VIP.", home_markup(chat_id))


def approve_vip_order(admin_id, order_id):
    if not _is_bot_admin(admin_id):
        send_message(admin_id, "⛔ Bạn không có quyền duyệt đơn.")
        return
    with BOT_VIP_ORDERS_LOCK:
        order = BOT_VIP_ORDERS.get(str(order_id))
        if not isinstance(order, dict):
            send_message(admin_id, "❌ Không tìm thấy đơn mua key.")
            return
        status = str(order.get("status"))
        if status != "PENDING_ADMIN":
            send_message(admin_id, f"ℹ️ Đơn {order_id} hiện ở trạng thái {status}.")
            return
        order["status"] = "PROCESSING"
        order["approved_by"] = str(admin_id)
        order["processing_at"] = _now_iso()
        _save_vip_orders()
    plan_id = str(order.get("plan_id"))
    success, data = _key_store_buy(order.get("telegram_id"), plan_id, order_id=order_id)
    if not success:
        msg = str(data.get("message") or data.get("error") or "Key Server không cấp được key.")
        with BOT_VIP_ORDERS_LOCK:
            order["status"] = "APPROVAL_FAILED"
            order["error"] = msg
            _save_vip_orders()
        send_message(admin_id, f"❌ DUYỆT THẤT BẠI\n\nĐơn: {order_id}\n{msg}")
        send_message(int(order["telegram_id"]), "❌ Yêu cầu của bạn đã được Admin xử lý nhưng Key Server chưa cấp được key. Vui lòng liên hệ Admin.", home_markup(int(order["telegram_id"])))
        return
    key = _extract_purchased_key(data)
    if not key:
        msg = "Server đã xử lý đơn nhưng không trả KEY."
        with BOT_VIP_ORDERS_LOCK:
            order["status"] = "APPROVAL_FAILED"
            order["error"] = msg
            _save_vip_orders()
        send_message(admin_id, f"❌ {msg}\n\nĐơn: {order_id}")
        send_message(int(order["telegram_id"]), "⚠️ Admin đã duyệt nhưng Key Server không trả key. Vui lòng liên hệ Admin.", home_markup(int(order["telegram_id"])))
        return
    with BOT_VIP_ORDERS_LOCK:
        ...
        _save_vip_orders()
    send_message(admin_id, f"✅ ĐÃ DUYỆT VÀ CẤP KEY\n\n🧾 Đơn: {order_id}\n👤 Telegram ID: {order.get('telegram_id')}\n🔑 Key: {key}")
    send_message(int(order["telegram_id"]),
                 "✅ YÊU CẦU DUYỆT KEY VIP THÀNH CÔNG!\n\n"
                 f"📦 Gói: {order.get('plan_name')}\n"
                 f"💰 Đã thanh toán: {int(order.get('amount', 0)):,} VNĐ\n"
                 f"🔑 KEY VIP: {key}\n\n"
                 "👉 Bấm 👑 NHẬP KEY VIP rồi gửi key này để kích hoạt.".replace(",", "."),
                 home_markup(int(order["telegram_id"])))


def reject_vip_order(admin_id, order_id):
    if not _is_bot_admin(admin_id):
        send_message(admin_id, "⛔ Bạn không có quyền từ chối đơn.")
        return
    with BOT_VIP_ORDERS_LOCK:
        order = BOT_VIP_ORDERS.get(str(order_id))
        if not isinstance(order, dict):
        ...
        if order.get("status") != "PENDING_ADMIN":
            send_message(admin_id, f"ℹ️ Đơn {order_id} hiện ở trạng thái {order.get('status')}.")
            return
        order["status"] = "REJECTED"
        order["rejected_by"] = str(admin_id)
        order["rejected_at"] = _now_iso()
        _save_vip_orders()
    send_message(admin_id, f"✅ Đã từ chối đơn {order_id}.")
    send_message(int(order["telegram_id"]), "❌ YÊU CẦU MUA KEY VIP CHƯA ĐƯỢC DUYỆT.\n\nVui lòng liên hệ Admin nếu bạn đã thanh toán.", home_markup(int(order["telegram_id"])))
...
def prompt_settings() -> bool:
    ...
    telegram_mode = os.getenv("VBTOOL_TELEGRAM_MODE") == "1"
...
if __name__ == "__main__":
    if len(sys.argv) >= 3 and sys.argv[1] == "--worker":
        ...
        run_worker(str(sys.argv[2]).strip().lower())
    else:
