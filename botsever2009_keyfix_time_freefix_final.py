...
def _create_free_short_link(telegraph_url: str) -> str:
    """Tạo link FREE qua Link4m -> Vượt Nhanh, có fallback khi Link4m lỗi."""
    ...
    # Bước 1: thử Link4m nếu đã cấu hình token.
    if LINK4M_TOKEN:
        try:
            r1 = requests.get(
                LINK4M_API,
            ...
            body = (r1.text or "").strip()[:500]
            if r1.status_code == 200:
                try:
                    data1 = r1.json()
                except ValueError:
                    data1 = {}
                if data1.get("status") == "success" and data1.get("shortenedUrl"):
                    link4m_url = str(data1["shortenedUrl"]).strip()
                else:
                    print(f"[FREE LINK] Link4m response lỗi: HTTP {r1.status_code} | {body}", flush=True)
            else:
                print(f"[FREE LINK] Link4m lỗi HTTP {r1.status_code} | {body}", flush=True)
        except Exception as exc:
            print(f"[FREE LINK] Link4m exception: {type(exc).__name__}: {exc}", flush=True)
    else:
        print("[FREE LINK] LINK4M_TOKEN chưa cấu hình -> bỏ qua Link4m.", flush=True)

    # Bước 2: Vượt Nhanh. Ưu tiên URL Link4m, nếu Link4m lỗi thì dùng Telegraph trực tiếp.
    target_url = link4m_url or telegraph_url
    if VUOTNHANH_TOKEN:
        try:
            r2 = requests.get(
                VUOTNHANH_API,
                params={"api": VUOTNHANH_TOKEN, "url": target_url},
                timeout=15,
            )
            ...
            if r2.status_code == 200:
                try:
                    ...
                    data2 = {}
                if data2.get("status") == "success" and data2.get("shortenedUrl"):
                    return str(data2["shortenedUrl"]).strip()
                print(f"[FREE LINK] VuotNhanh response lỗi: HTTP {r2.status_code} | {body2}", flush=True)
            else:
                print(f"[FREE LINK] VuotNhanh lỗi HTTP {r2.status_code} | {body2}", flush=True)
        except Exception as exc:
            print(f"[FREE LINK] VuotNhanh exception: {type(exc).__name__}: {exc}", flush=True)
    else:
        print("[FREE LINK] VUOTNHANH_TOKEN chưa cấu hình -> dùng Telegraph trực tiếp.", flush=True)

    # Không trả Telegraph trực tiếp: người dùng phải đi qua Vượt Nhanh để lấy Key.
    raise RuntimeError("Vượt Nhanh không tạo được link lấy Key FREE")
...
def create_free_key_link(device_id: str) -> Optional[str]:
    ...
    try:
        # Link4m có thể trả HTTP 500; helper sẽ tự bỏ qua và dùng Vượt Nhanh/Telegraph.
        final_url = _create_free_short_link(telegraph_url)
        if not final_url:
            console.print("[bold red]❌ Không tạo được link lấy Key.[/bold red]")
            return None
